...
This appendix fixes the second post-Newtonian (2PN) Shapiro
signature of the bi--conformal ISPG geometry in the operational
form relevant for comparison with GR.  It proves the exact
straight-line refractive sub-piece and, more importantly, closes
the finite ISPG--GR \emph{differential} coefficient.  The three
pieces entering an absolute 2PN light time are:
...
and~(iii) are individually convention-dependent unless one
chooses a complete finite-endpoint time-transfer prescription.
For the ISPG--GR comparison they do not remain as unresolved physics:
their dependence is entirely on the shared $O(r_g)$ trajectory
of the two metrics, so they cancel in the difference.  The
differential 2PN coefficient is therefore finite, IR-insensitive,
and computed exactly in Sec.~\ref{sec:diff}, with the closed form
\[
...
distinction.  Absolute finite-endpoint coefficients for each
metric separately belong to a chosen timing convention; they are
not needed for the differential discriminator closed here.
...
to the absolute-coefficient bookkeeping in Sec.~\ref{sec:open}.
...
\section{Common bent-ray and path-length terms}\label{sec:open}

The two remaining 2PN contributions involve the
$O(r_g)$ trajectory correction.  For an absolute coefficient
assigned to one metric in isolation they require a definite
finite-endpoint time-transfer convention.  For the ISPG--GR
differential coefficient they are common terms: both metrics
share the same 1PN optical index and therefore the same
$O(r_g)$ ray shape.  We record the framework to show why the
individual pieces are not separately meaningful in the
unregularized decomposition and why they cancel in the
...
\paragraph{The infra-red issue for individual coefficients.}
...
Sec.~\ref{sec:finite} below.  The analogous absolute
finite-distance treatment for~\eqref{eq:bent_integral}--\eqref{eq:path_integral}
is a timing-convention problem; it is common to both metrics
at the differential order considered here.

\paragraph{Consequence for the main theory.}
The main theory does not need the separate absolute values of
the bent-ray and path-length pieces to make its 2PN Shapiro
prediction.  Since these terms are generated by the shared
1PN trajectory, they cancel in the ISPG--GR subtraction.
The closed prediction is therefore the finite differential
coefficient derived in Sec.~\ref{sec:diff}.
...
belong to the absolute time-transfer convention and cancel
from the ISPG--GR differential coefficient.
...
coefficients, even though each individual absolute coefficient
is convention-dependent in the present decomposition
(Sec.~\ref{sec:open}).
...
The individual absolute 2PN coefficients $B_{\rm ISPG}$ and $B_{\rm GR}$
are each IR-divergent at intermediate stages of the
decomposition~\eqref{eq:2pn_full} (Sec.~\ref{sec:open});
their standalone values require a specified finite-endpoint
time-transfer convention.  The
...
\emph{individual} absolute 2PN coefficient are convention-dependent
(Sec.~\ref{sec:open}).  Their dependence on the shared $O(r_g)$
trajectory makes them identical in ISPG and GR, so the
