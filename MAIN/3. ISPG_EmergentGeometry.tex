...
\paragraph{Measurable-existence identification.}
The same postulates also admit an operational identification.  Local clocks
...
matter-standard response is denoted
\[
  \frac{\mathcal N_{\rm act}(\phi)}
       {\mathcal N_{\rm act}(0)}
  = e^{\phi/2}.
\]
Here $\mathcal N_{\rm act}$ is not an independent field, number-density
count, or three-dimensional volume count of medium nodes.  It is the
operational resonant-participation content of the matter standard: the
active measurable content whose projections appear as clock rate, rod scale,
and effective mass.  This identification adds no new dynamics and does not
replace the clock/rod proof of local light-speed invariance.
