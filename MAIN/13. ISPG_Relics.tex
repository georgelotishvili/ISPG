...
The main theory derives the 1PN Shapiro delay via an effective refractive index $n(r)=e^{-\phi}$ (identical to GR at 1PN, as $\gamma=1$) and identifies a structurally distinct 2PN signature from the exponential $n=e^{-\phi}$; the refractive sub-piece of the 2PN delay is derived exactly and the closed ISPG--GR difference $\Delta B = \pi/4$ is established at the regularization-independent level, while the absolute 2PN coefficient (including bent-ray and path-length contributions) is deferred to future work (the main theory Shapiro section~\cite{Lotishvili2026} and Appendix~6). For later use we record the dependency structure:
\begin{equation}
...
A central design goal of the main theory is that \emph{linear} cosmology can remain GR-identical while \emph{nonlinear} bound structures exhibit new behaviour. The separation is operationally expressed by a scale hierarchy:
\begin{equation}
...
Bi--conformal metric \eqref{eq:biconf_app13}; covariant action \eqref{eq:action_app13}; weak-field $\gamma_{\rm PPN}=1$; Shapiro 1PN with a 2PN discriminator; linear-cosmology GR-identity under $\alpha_i=0$ (the main theory and CMB appendix); spectator sourcing \eqref{eq:deltaphi_app13}; MOND interpolating function $\mu(x)=x/(1+x)$ from the bound-structure Bernoulli$+$vortex closure (the companion MOND paper~\cite{Lotishvili2026MOND}).\\
\midrule
