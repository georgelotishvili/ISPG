...
to that of \LCDM{} with the same matter content and initial conditions. Therefore the
primary CMB angular power spectrum, BAO scale, supernova distance measures, and
BBN expansion-rate constraints are inherited at linear order. The distinguishing
...
cosmological scalar background is \emph{forced} to vanish by simultaneous imposition of the FLRW
...
independent dynamical prediction. This appendix makes this point explicit to avoid any ambiguity.
...
mode; under $\alpha_i=0$ it does not back-react on the metric potentials at
linear order.
...
decoupled from the metric potentials at linear order under $\alpha_i=0$. The
...
GR-identical, the following are inherited at linear order: primary CMB anisotropies,
BAO standard ruler, supernova distance measures, and the BBN expansion-rate constraint
$H(T)$.
...
Primary CMB $C_\ell$ & Unchanged (GR-identical linear sector) \\
BAO scale & Unchanged (same background + linear perturbations) \\
SNe distances & Unchanged (same background distances) \\
BBN $H(T)$ & Unchanged (no background scalar contribution) \\
Linear growth $f\sigma_8$ & Unchanged (GR-identical linear sector) \\
Lensing potential $(\Phi+\Psi)/2$ & Unchanged at linear order \\
\bottomrule
\end{tabular}
\end{table}
