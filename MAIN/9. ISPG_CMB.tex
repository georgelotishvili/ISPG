...
identically ($\aK=\aB=\aM=\aT=0$); together with the absence of a term linear in
$\delta\phi$ in the scalar stress tensor, the linear Einstein--Boltzmann system
reduces to that of \LCDM{} with the same matter content and initial conditions. Within this
...
\label{eq:alphazero}
\end{equation}
This vanishing is a metric-sector EFT statement on the
zero-background-kinetic branch; it does not by itself certify
scalar no-ghost stability, which is established covariantly in
Appendix~11.
...
$\phi_{\mathrm{bg}}(z)$.  The corresponding late-time/early-universe
regime separation is discussed in Appendix~\ref{sec:flrw_vs_process_app18},
and the pressure-history multiplier used for process-time bookkeeping is
defined in Eq.~\eqref{eq:pressure_history_C}.
...
Primary CMB $C_\ell$ & Unchanged in the same-matter-content metric-sector limit \\
BAO scale & Unchanged in the same-matter-content metric-sector limit \\
SNe distances & Unchanged for the same background distances in this limit \\
BBN $H(T)$ & Unchanged in the same-matter-content metric-sector limit \\
Linear growth $f\sigma_8$ & Unchanged in the same-matter-content metric-sector limit \\
...
ISPG bi-conformal (this work) & $0$ & $0$ & $0$ & same-matter metric limit \\
