...
\item reproduces the classical 1PN solar-system tests
  ...
  and predicts a
  structurally distinct 2PN Shapiro-delay signature
  (arising from the exponential bi-conformal refractive
  index); the exact 2PN coefficient awaits a full
  finite-endpoint derivation and is flagged as future work
  in Sec.~\ref{sec:shapiro};
...
at 1PN order (where ISPG matches GR exactly) and sketches
the structural 2PN signature induced by the bi-conformal
exponential refractive index, while flagging the complete
2PN coefficient as open (future work).
...
\textit{Bent-ray optical and excess path-length contributions
(in progress).}---At 2PN order the photon follows a bent
trajectory corrected at $O(r_{g})$, and the bending produces
both (i) a shift of the $O(r_{g})$ refractive integrand along
the corrected path, and (ii) an excess Euclidean length of the
bent spatial path.  A \emph{rigorous} derivation of these two
contributions requires (a) a consistent convention for the
asymptotic impact parameter $b$ versus the geometric closest
approach, and (b) a careful treatment of the finite
emitter/receiver endpoints $r_{1},r_{2}$, because individually
each piece is infra-red sensitive (log/linear in the endpoint
radii) and only the full combination is finite.  A self-contained
such derivation in the bi-conformal ISPG geometry is deferred
to future work.
...
numerically.  The \emph{ISPG--GR difference} $\Delta B = \pi/4$
is closed at the regularization-independent level (Appendix~6:
the bent-ray and path-length pieces are common to both metrics
and cancel in the difference, while the refractive coefficients
$\alpha_{\rm ISPG} = 2$ vs $\alpha_{\rm GR} = 7/4$ leave a clean
$(\alpha_{\rm ISPG}-\alpha_{\rm GR})\pi = \pi/4$ residual).
The \emph{absolute} 2PN coefficient---which requires
finite-endpoint analysis of the individually infra-red-sensitive
bent-ray and path-length contributions---is flagged here as an
open task.  What is theoretically secure at this stage is
therefore: (i) the regularization-independent ISPG--GR
discriminator $\Delta B = \pi/4$, and (ii) the
\emph{structural} claim that the bi-conformal exponential
$n = e^{-\varphi}$ produces a 2PN Shapiro signature distinct
from GR's rational-polynomial refractive index.  The
absolute-coefficient observational prospects depend on the
...
uncertainty) and Sgr~A\textsuperscript{*} ($51.8\pm 2.3\,\mu$as,
...
a homogeneous scalar field $\varphi_{0}(t)$ satisfies
...
forces $e^{\varphi_{0}} = 1$, i.e.\ $\varphi_{0} = 0$
...
Since the gravitational potentials obey exactly the
GR equations, the CMB angular power spectrum at linear
order is
...
GR-identical cosmology (Sec.~\ref{sec:cmb}), and (ii)~the
...
the homogeneous background pressure history controls the
cosmological sector, while bound rotating structures excite
the local Bernoulli and rotational response channels that
generate MOND.  The companion MOND paper~\cite{Lotishvili2026MOND} develops this local
...
\item Full numerical ray-tracing for 2PN Shapiro delay in
  Eq.~(\ref{eq:metric_static}) is future work.
...
\item \textbf{2PN Shapiro (structural):} the bi-conformal
  exponential refractive index $n=e^{-\varphi}$ produces a
  2PN delay signature distinct from GR's rational-polynomial
  form; the refractive piece yields an exact $2\pi$ coefficient.
  A complete 2PN coefficient (including bent-ray and
  path-length contributions with consistent finite-endpoint
  regularization) is flagged as future work
  (Sec.~\ref{sec:shapiro}).
