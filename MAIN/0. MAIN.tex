...
\item proves the scaling $\azero \propto c\,H_{0}$ from
  ...
  paper~\cite{Lotishvili2026MOND} selects the
  Hubble-wavelength normalization $1/(2\pi)$ together with the
...
($\eta_{\mathrm{E\ddot{o}t}}$ constrained at the
$10^{-15}$ level);
...
the same-matter-content metric-sector limit of App.~9 gives
the standard metric equations: $\alpha_i = 0$ removes the
usual EFT modifications, while
$\partial_\mu\varphi_0 = 0$ makes the scalar stress-energy
quadratic in $\delta\varphi$.
...
not back-react on $\Phi,\Psi$ at linear order: $\alpha_i = 0$
fixes the metric-sector EFT functions, and the App.~9
stress-order decoupling
$\partial_\mu\varphi_0 = 0$ removes any scalar stress term
linear in $\delta\varphi$.  The mode is not a free spectator;
it is
...
$\omega_{k}^{-1}$ in the deep WKB / short-wavelength
range $kc/a\gg H$.  In that range the homogeneous equation
is a controlled regime-classification device.  Near the
Hubble-scale boundary $kc/a\sim H$, however, this
source-drop is only a classification approximation; the
full driven response is written in~(\ref{eq:scalar_pert})
and analysed in App.~9.  The homogeneous form is
...
= 2kc/(3aH)$ organizes the homogeneous regimes:
\begin{itemize}
\item $kc/a\gg H$ (deep WKB / short-wavelength range,
  hence $Q\gg1$): oscillatory---the scalar field sustains
  pressure gradients and provides full inertial response;
\item $kc/a\sim H$ (near-Hubble / order-Hubble boundary,
  hence $Q=O(1)$): the homogeneous oscillator crosses from
  oscillatory to overdamped behaviour; this identifies an
  $O(c/H)$ coherence boundary, not a direct Hubble-friction
  damping of local galactic modes;
\item $kc/a \lesssim H$ (super-Hubble): the adiabatic
  ...
  Hubble radius.
\end{itemize}
The order-Hubble transition occurs at
...
Hubble friction $H$.  As a homogeneous FLRW classification,
it identifies a longest order-Hubble coherence wavelength;
galaxy-scale modes with $kc/a\gg H$ are not directly
overdamped by the FLRW friction term.  The MOND use below
therefore treats this scale as a global coherence-boundary
input to the bound-structure closure.  The corresponding
wavelength is set by the Hubble rate through the standard
dispersion relation:
...
extends beyond the adopted Hubble coherence length---the
bound-structure closure no longer assigns a full Newtonian
response to the scalar channel.
...
\end{equation}
Here $H_{0}=67.4\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$
is the fiducial Planck/CMB baseline value used in
the numerical estimate and in
\texttt{MOND/Python/constants.py}.  Since
$\azero\propto H_{0}$, using a local-distance-ladder
value near $73\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$ would
rescale this instantaneous estimate upward by about
$8\%$.

\textbf{Demonstrated at H.1 level:} the scaling
$\azero \propto cH_{0}$ follows from the
order-Hubble coherence boundary identified by the damped
...
therefore selected by the coherence geometry rather than fit
to data, and the PDE verification therein checks the same
normalization within the bound-structure closure.
...
mechanism-level consistency filter complementary to the
...
are therefore inconsistent with the vortex-lag closure.
...
\item \textbf{Elliptical galaxies.}  The companion MOND
  ...
  $g\sim a_0$ selected by the cosmological coherence length.
...
\begin{equation}\label{eq:F_grav}
  ...
  = -m\,\nabla\Phi_{N,\mathrm{ext}}\,,
  \qquad
  \Phi_{N,\mathrm{ext}}
  \equiv \tfrac{c^{2}}{2}\,\varphi_{\mathrm{ext}}\,,
\end{equation}
...
tested facts in physics: the MICROSCOPE
experiment~\cite{MICROSCOPE2022} constrains the
...
In GR with minimally coupled matter the equality is automatic at the
...
explicitly in Appendix~17, which obtains the inertial
response $\mathbf{F} = -m\,\mathbf{a}$ in the
weak-field, slow-motion, regularised point/compact-source
regime, with the finite regular-field coefficient
$C_{\mathrm{reg}}$ and the spatial-sector Noether
momentum derivation explicitly deferred to residual
technical tasks.  The quantum
extension~\cite{Lotishvili2026Q} provides a parallel
microscopic localized-resonance account via the
Bernoulli pressure-deficit mechanism, with leading
projection $C_{\mathrm{in}}=1$ and Bernoulli
normalization $E_\varphi = m_{\mathrm{eff}} c^{2}$ on
the spherical equilibrium oscillon profile; the
combined coefficient closure across both calculations
remains an open numerical task.
...
\begin{equation}\label{eq:meff_def}
  \boxed{m_{\mathrm{eff}}(\varphi)
    = m_{0}\,e^{\varphi/2}\,.}
\end{equation}
In the measurable-existence identification, this is the asymptotic
mass/energy projection of the same active measurable content that defines
the local matter standard.  The identification introduces no new source term:
the geometric and dynamical content remains the bi-conformal scaling above.
...
Eq.~(\ref{eq:c_local_meas}); the locally measured rest
mass of the particle, weighed against the local reference
kilogram, remains the bare value $m_{0}$.

In the coordinate bookkeeping, where lengths and times
are referred to a fixed asymptotic chart rather than to
local proper standards, all reference standards share
the same conformal factor $e^{\varphi/2}$: the test
particle, the reference kilogram, and the atoms in the
scale all carry $m_{\mathrm{eff}}=m_{0}e^{\varphi/2}$.
The ratio of any two coordinate effective masses is
...
and the locally measured rest masses themselves stay at
the bare values $m_{0}$.
Operationally, the object and the apparatus co-scale because both are built
from the same interaction-active measurable content; only nonlocal or
cross-epoch comparisons can expose the common scaling factor.
...
here $P_{\max}$ is the static/asymptotic reference
normalization of the isolated-source pressure variable,
not a strict upper bound imposed on the coarse-grained
cosmological pressure-history diagnostic.  The
cosmological branch below is expressed in ratios to
$P_{\mathrm{stat}}(0)$.
...
\begin{equation}\label{eq:pressure_history_C}
  ...
  = \left[
    ...
    \right]^{1/2}.
\end{equation}
Here $\mathcal N_{\rm act,bg}$ is the background form of the
measurable-existence identification, attached to the thermodynamic
pressure-history diagnostic
$\varphi_{\rm bg}$, not to a second homogeneous FLRW metric scalar.  It is
not an independent number-density count and introduces no new directly
measured observable; it only names the same pressure-scaling ratio already
carried by $\mathcal C(z)$.
With this convention the common pressure-history scaling is
\begin{equation}\label{eq:early_scaling}
\renewcommand{\arraystretch}{1.4}
\begin{array}{rcll}
  \mathcal N_{\rm act,bg}(z)/\mathcal N_{\rm act,bg}(0)
    &=& \mathcal{C}(z), \\
...
\item \textbf{Cosmology:}
  $\alpha_K=\alpha_B=\alpha_M=\alpha_T=0$ on FLRW, together
  with the App.~9 stress-order decoupling, gives linear CMB
  equivalence with $\Lambda$CDM in the same-matter metric-sector
  limit (Sec.~\ref{sec:cmb}).
