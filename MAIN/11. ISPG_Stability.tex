...
\end{equation}
The remaining function $\alpha_K$ is proportional to the background kinetic density $X$ for this minimal subclass. On the FLRW background used in the CMB appendix, the consistency condition yields $\dot{\phi}_0=0$ and hence $X_0=0$, so $\alpha_K=0$ on that background. Therefore the linear-cosmology sector is GR-identical as claimed.
