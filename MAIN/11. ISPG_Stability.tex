...
The vanishing of $\alpha_K$ on the cosmological background does \emph{not} indicate a pathology; it reflects that the homogeneous mode is fixed by the bi--conformal consistency condition. On this $\dot\phi_0=0$ branch the standard scalar-clock / unitary-gauge EFT variable $\zeta$ is degenerate: $Q_s=0$ in Eq.~\eqref{eq:QsBS} reflects the absence of a homogeneous scalar clock, not a positive no-ghost certificate for the propagating or matter-sourced scalar. Stability for that scalar sector is established covariantly through the principal-symbol analysis above, the constrained-sector argument below, and the EFT completion of the phantom matter channel. In the bound-structure sector (quasi-static, inhomogeneous configurations) the scalar is sourced and the kinetic operator is the standard hyperbolic one encoded by the principal symbol, ensuring well-posed evolution.
