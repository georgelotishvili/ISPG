...
Here $P_{\max}$ denotes the static/asymptotic reference
normalization of the isolated-source pressure variable,
not a strict upper bound on the coarse-grained cosmological
pressure-history diagnostic.  Cosmological pressure histories
below are expressed as ratios to $P_{\mathrm{stat}}(0)$ and
can exceed this reference ratio in the past.
...
\end{equation}
In the measurable-existence identification, the same factor may be written as
\begin{equation}
  \mathcal{C}(z)
  =
  \frac{\mathcal N_{\rm act,bg}(z)}
       {\mathcal N_{\rm act,bg}(0)}.
  \label{eq:Nact_bg_app18}
\end{equation}
Here $\mathcal N_{\rm act,bg}$ is identified operationally through this ratio:
it is not an independent number-density count, not a three-dimensional
volume count of medium nodes, and not a new directly measured observable.
It is attached to the coarse-grained pressure diagnostic
$\phi_{\mathrm{bg}}$, not to a second homogeneous FLRW metric scalar.
...
measurable-existence ratio in \eqref{eq:Nact_bg_app18} is a matter-standard
scaling factor, not an automatic extra multiplier on rates or powers already
written per coordinate time, local proper time, or observer-calibrated time.
The full non-perturbative tail power should be written schematically as
\begin{equation}
...
\item \textbf{$\phi_0$: the homogeneous metric scalar.}  This is the
  ...
  cosmic-time gauge it is fixed to vanish at every epoch
...
\midrule
Past   & finite ($t_0\simeq 13.8$ Gyr)
...
$e^{-3H(t-t_0)}$ at late times in this asymptotic scenario.  Under
this assumed late-time exponential decay, during the transient
...
duration is of order $H_0^{-1}$, with $H_0^{-1}\simeq
14.5\;\mathrm{Gyr}$ for the fiducial background (distinct from the
corresponding coordinate age $t_0\simeq 13.8\;\mathrm{Gyr}$).
...
are therefore of the same order-Hubble scale, supporting the
anti-diagonal structure of the duality table under the stated
late-time decay scenario while keeping $t_0$ numerically distinct
from $H_0^{-1}$.

\paragraph{Physical reading.}
If the duality conjecture is exact, the universe has no operational
time before the first resonant clocks and no finite-time terminal
shutdown.  The past has a finite coordinate age and, under the
fiducial closure, a finite mathematical process-time integral; the
pre-resonance regime is clockless rather than an infinite measured
past.  The future has unbounded coordinate time, while the remaining
process-time budget is finite only if the convergence condition above
is satisfied.  The ``Big Bang'' and the ``heat death'' are therefore
read here as transitions between temporal regimes, not as completed
derivations of an infinite proper-time past or a finite terminal
future.
The de~Sitter / infinite-coordinate-future reading is consistent with
the source-driven phantom regime only under the asymptotic condition
$\delta w\to 0^{+}$, so that $w\to -1$ from the phantom side, as
derived in the companion quantum paper (Sec.~\ref{sec:dark_energy}).
...
Active measurable content & $\mathcal N_{\rm act}(\phi)$ &
  $\mathcal N_{\rm act}/\mathcal N_{\rm act}(0)=1$ locally \\
...
\item \textbf{Coordinate view of the past}: process clocks were fast,
  masses were larger, emission was stronger.  The amount of
  resonant-sector activity is the closure-dependent process duration
  accumulated over the finite coordinate interval $t_0$; under the
  fiducial benchmark this mathematical past integral is finite.
...
the future process-time budget
$T_{\mathrm{int,\,future}}$ (finite only if its convergence condition
holds) occupy anti-diagonal positions in the
