...
საკითხი:
  არსებული ფაქტი (phase_a_gauge_sector.py):
    ...
    (N_W/N_Z)² = 0.8818 ≈ m_W/m_Z = cos θ_W(on-shell) = 0.8815
    (0.03% ცდ.)
...
ზუსტად (N/5)² მასის ფორმულასთან ემთხვევა. ეს target არის
m_W/m_Z = cos θ_W on-shell convention, არა cos² θ_W.
...
""")
    ...
    target = 80369/91188
    print("  ახლო-მყოფი (N_W, N_Z) წყვილები: ცდ. (N²-მოდელი) vs PDG m_W/m_Z = cos θ_W")
    print("  " + "-"*70)
    best_fit = []
...
""")
    target = 80369/91188
    print(f"მიზანი: effective mass-ratio proxy ≈ {target:.4f} = m_W/m_Z = cos θ_W")
    print()
...
რა გამოდის (გულახდილი):
  ...
  • Mathieu a_N(q) დიდ N-ზე ≈ N², ე.ი. cosθ_W = (N_W/N_Z)²
    mass-ratio convention-ში ავტომატურად უკავშირდება ნებისმიერ q-ს.
