...
print("\n" + "-" * 72)
print(" C. 59 = κ²_τ/κ²_e ÷ (N_e × 1/N_e) ... ე.ი. პირდაპირ:")
print("-" * 72)

# თუ N_τ = 5·(κ²_τ/κ²_e) = 295, მაშინ κ²_τ/κ²_e = 59
#   ე.ი. 59 არის თავისთავად 3D კავიტის κ² ფარდობა!

print(f"""
   ანზაცი: N_τ/N_e = 59 ⇔ κ²_τ/κ²_e = 59

   ჩვენი 3D-ის გამოთვლა: κ²_τ/κ²_e = {ratio_tau_e:.4f}

   **ე.ი. 59 = 3D პულსონის ყველაზე ღრმა (ℓ=0,n=0) მდგომარეობის
   ბმულობის ენერგია ℓ=2 მდგომარეობასთან შეფარდებით.**
""")

close = abs(ratio_tau_e - 59) / 59 * 100
print(f"   გამოთვლა: {ratio_tau_e:.4f} → 59-თან ცდომილება {close:.2f}%")
...
print(f"""
   ცდომილება: {abs(ratio_tau_e - 59)/59*100:.2f}%
   შესაძლო მიზეზები:
     1. ALPHA_NL = 0.5 ფიტირებული პარამეტრია — არ ზუსტი
     2. Φ_C = 2.35 — ფიტირებული
     3. 3D რადიალური ODE რეგულარიზაცია r=0-ზე
     4. κ² ფარდობა დონე-ზუსტი, N_τ=295 შესაძლოა 5·κ² კვარი იყოს

   თუ ეს 0.9% ცდომილება ფიზიკური არ არის, მაშინ არც 59 არ არის მთელი.
   (59 არის 295/5 = 59.0 ემპირიული ანზაცის მიხედვით.)

   თუ ნამდვილი ანალიზი იძლევა N_τ = 5 · 58.4 = 292,
   მაშინ შეიძლება 295 თავად იყოს ფიტი და ნამდვილი რიცხვი 292-ს უფრო ახლოა.
""")
...
print(f"""
   **ახალი ჰიპოთეზა რომელიც ახლა შემოწმდა:**
   ...
   რიცხვითი შედეგი:
   ─────────────────
   κ²_τ/κ²_e = {ratio_tau_e:.2f}       (ელოდა: 59, ცდომილ. {abs(ratio_tau_e-59)/59*100:.1f}%)
   κ²_μ/κ²_e = {ratio_mu_e:.2f}        (ელოდა: 14.4, ცდომ. {abs(ratio_mu_e-14.4)/14.4*100:.1f}%)

   რასაც ვხედავთ:
   ─────────────────
   • ტაუ ძალიან ახლოა 59-ს (0.9% სიზუსტით)
   • მიუონი: {ratio_mu_e:.2f} vs 14.4 (~{abs(ratio_mu_e-14.4)/14.4*100:.0f}% ცდომილება)

   ე.ი. **ტაუ რიცხვი 59-ში ჩანს 3D პულსონიდან პირდაპირ!**
   ეს არის 59-ის ფიზიკური წარმოშობა — პულსონის ℓ=0,n=0 / ℓ=2,n=0
   ბმულობის ენერგიის ფარდობა.

   მიუონის ცდომილება შეიძლება:
     — ℓ=0,n=1 არ არის სწორი მიუონის მიკუთვნება
     — ან ℓ=0,n=1 და სხვა ℓ=1 ერთად (შერევა?)
""")
