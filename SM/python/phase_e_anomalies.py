...
ISPG-ს კონსისტენცია: imported SM Gen-1 Q, T₃, Y assignment
ზუსტად გადის anomaly checks-ს. ეს არის consistency check,
არა ჰიპერმუხტების ISPG-დან გამოყვანა.
"""
...
ინტერპრეტაცია ISPG-სთვის:
  ზემოთ გავლილი ხუთი ტესტი არის exact consistency check
  imported SM Gen-1 hypercharges/chiralities-ზე. ეს არ არის
  SM ჰიპერმუხტების ISPG-დან გამოყვანა; ეს აჩვენებს, რომ
  იმპორტირებული SM განაწილება ISPG Gen-1 content-ზე
  ანომალიებთან კონფლიქტში არ შედის.

ISPG-ისთვის **სპეციფიკური** შემოწმება (Phase C Step 2-დან):
  Y · N_c ∈ {−2, −1, +1, +4} — ყველა **მთელი**

  ეს არის charge-quantization / integer-grading compatibility
  clue, რომელიც anomaly cancellation-ის გვერდით ჩანს იმპორტირებულ
  SM განაწილებაში. მაგრამ ეს არ არის თეორემა, რომელიც თვითონ
  აუქმებს ანომალიებს: საჭიროა ზემოთ მოცემული ხუთი განტოლების
  შემოწმება კონკრეტულ მულტიპლეტებზე და ჰიპერმუხტებზე.
  მაგალითად, ყველა Y-ის საერთო არარაციონალური rescaling ინარჩუნებს
  anomaly sums = 0-ს, მაგრამ Y · N_c აღარ იქნება მთელი; ხოლო ერთი
  Y-ის integer perturbation შეიძლება დატოვოს Y · N_c მთელი, მაგრამ
  U(1)_Y³ ჯამი შეცვალოს.

ამიტომ Phase E = exact imported-SM anomaly consistency check;
hypercharge origin open.
""")
