...
if (2, 0) not in results or (0, 0) not in results:
    print()
    print("=" * 72)
    print(" Phase 16 — Floquet ⟨V⟩_t უარყოფითი დიაგნოსტიკა")
    print("=" * 72)
    print(f"""
   ნაივური დროის-საშუალო პოტენციალი ⟨V⟩_t კარგავს საჭირო
   ბმულ მოდს: (2,0) ან (0,0) bound state ⟨V⟩_t-ში არ არსებობს.

   მიზეზი: cos(Ωt)-ის უარყოფით პიკზე exp(−αΦ·cos(Ωt))
   ექსპონენციურად იზრდება, ამიტომ ⟨exp(−αΦ·cos(Ωt))⟩ = I_0(αΦ)
   r=0-ში მოგერიებელი ხდება, ნაცვლად სტატიკური მიმზიდველი
   პოტენციალისა.

   დასკვნა: მარტივი ⟨V⟩_t-აპროქსიმაცია ამ რხევის რეჟიმში
   (Ω = {Omega:.3f}, αΦ ≈ {ALPHA*PHI_C:.3f}) არ ხურავს 0.9%
   ხიდს. სრული Floquet-Bloch ოპერატორი საჭიროა — ცალკე
   numerical task, ახალი ფიზიკის შემოტანის გარეშე.
""")
    print("=" * 72)
    print(" Phase 16 — დასრულდა (უარყოფითი დიაგნოსტიკა)")
    print("=" * 72)
    raise SystemExit(0)
