...
the ISPG pulson substrate and the Mathieu ladder previously
...
matches $\cos\theta_W=0.8815$ to $0.03\%$—an
\emph{internal consistency check}, not an independent
prediction of $\cos\theta_W$, since the two indices are
themselves fits to the masses whose ratio defines
$\cos\theta_W$ at SM tree level;
...
\emph{necessary} condition for anomaly cancellation that the
ISPG cavity labels naturally enforce; the \emph{sufficient}
...
vibration axes of the 3D pulson, giving a geometric reading of
the number $3$;
...
structure beyond the count; the pulson-interior mechanism for
...
potential; (b) localized finite-energy solutions (``pulsons'') exist
as time-periodic oscillons inside a finite cavity; (c) the linear
spectrum of small fluctuations around the pulson obeys Mathieu's
...
\paragraph{Pulson and $D_3$.} \fromISPG{\cite{Lotishvili2026Q}}
A pulson with triangular ($D_3$) deformation in three spatial
...
\texttt{.cursor/rules/ISPG\_EightGluon\_Robustness.md});
this is used in \S\ref{sec:phaseA}.

\paragraph{Three-axis hypothesis.} \numerical\ \modeldep\
The 3D pulson can sustain three orthogonal vibration modes
...
the linearized pulson spectrum. The large-$N$ approximation
...
\derived\ (count only; documented in the robustness analysis,
supplementary notes on file with the author; summarized here).
...
eight are confined (lie below the continuum threshold) across a
...
$\mathrm{SU}(3)$ adjoint representation ($\dim=8$) without fine
tuning.
...
the nonlinear pulson dynamics (specifically, from the
...
Particle & $m$ (GeV) & $N$ & $m_{\text{pred}}=(N/5)^2 m_e$ &
  error \\
\midrule
$W^\pm$ & $80.369$ & $1986$ & $80.12$\,GeV & $0.31\%$ \\
...
\begin{equation}
  ...
  \;=\;0.8817,
\end{equation}
is to be compared with $\cos\theta_W=0.8815$
...
constitute an independent prediction of $\cos\theta_W$; at SM
tree level $m_W/m_Z=\cos\theta_W$ holds identically and any
...
Green on the gluon count: the $D_3$-deformed oscillon reproduces
$\dim\mathrm{SU}(3)_{\mathrm{adj}}=8$ robustly, though the full
Lie-algebra structure is deferred (E6). Yellow on
...
independent prediction of $\cos\theta_W$. The derivation of the
...
\subsection{Generation-1 cavity-to-SM map}
\label{sec:phaseC_gen1}

\numerical\ We look for a map
$(\text{cavity labels})\to(Q,N_c)$ that reproduces the
generation-1 Mathieu indices for the massive charged fermions
$(e,u,d)$. An ansatz using only cavity-native quantities
\begin{equation}
...
$\mathbb{Z}_3$ quantum number natively expressible on the cavity.
...
is the \emph{shape}: that a formula built only from
cavity-native quantities $(Q,N_c)$, anchored on the electron
and returning integer multiples of $5$, exists at all. A
...
pulson dynamics. Both are deferred.
...
\paragraph{Integer hypercharge-times-color.} \derived\ A direct
reading of the last-but-one column gives
\begin{equation}
  \label{eq:YNc_integer}
  Y\cdot N_c \;\in\; \{-4,\,-2,\,-1,\,+1,\,+2\}\;\subset\;\mathbb{Z}
\end{equation}
for every field (including both chiralities). This
integrality is a \emph{necessary} condition for the SM gauge
anomalies to cancel (\S\ref{sec:phaseE}): any assignment with
a non-integer $Y\,N_c$ would fail at least one anomaly sum. It
is \emph{not} sufficient: the anomalies require the specific
set $\{-4,-2,-1,+1,+2\}$ with the specific multiplicities shown,
not merely arbitrary integers. The ISPG cavity framework
provides the necessary integrality automatically---because
$Y$ and $N_c$ are both cavity labels and their product respects
the integer-grading of the cavity spectrum---but the specific
values are imported through the Gen-1 formula
\eqref{eq:gen1formula} combined with Gell-Mann--Nishijima;
they are not independently derived from pulson dynamics in
this work. The integrality is also a hallmark of
...
fit formula~\eqref{eq:gen1formula} built from cavity-native
quantities $(Q,N_c)$; the fit is exact on the three data
...
it. The integer structure $Y\,N_c\in\mathbb{Z}$ is a necessary
condition naturally respected by cavity labels.
...
\subsection{Local EW breaking: the pulson interior}
...
each pulson}. A pulson is a localized finite-energy excitation
with $\Phi(r)\neq 0$ inside its core and $\Phi(r)\to 0$ outside.
The SM's ``broken phase'' maps to the pulson interior; the SM's
...
from the pulson interior---the analogue of the Higgs Yukawa
...
$\mathrm{SU}(2)$ gauge field minimally coupled to the pulson
acquires a mass matrix matching $M_W,M_Z$ (with the photon
remaining massless) from the pulson's spatial profile alone.
...
\begin{itemize}
  ...
  \emph{internal length scale} of the pulson, not a property of
...
\subsection{Consistency check: $m_W/m_Z=\cos\theta_W$}

\numerical\ As discussed in \S\ref{sec:phaseA_ew}, the
ratio~\eqref{eq:cosThetaW} matches $\cos\theta_W$ to
$0.03\%$. This is a self-consistency check of the Mathieu
ansatz (the same integer ladder hosts both bosons), not an
independent prediction of $\cos\theta_W$, since the individual
...
pulson) is proposed but not derived (E7). The $m_W/m_Z$ ratio
...
\derived\ The anomaly conditions are conditions on the
\emph{precise} hypercharge assignment: any other
$(Y_{L_L},Y_{Q_L},Y_{e^c},Y_{u^c},Y_{d^c})$ choice reactivates
at least one anomaly. The ISPG cavity framework must therefore
support \emph{exactly} these $Y$ values. Phase~C furnishes the
integrality~\eqref{eq:YNc_integer}, which is a
\emph{necessary gate-keeper}: it rules out any non-integer
cavity assignment. The \emph{sufficient} SM-specific values
...
pulson dynamics in this paper. What Phase E therefore
establishes is: the SM hypercharge assignment, once imported
onto the ISPG cavity labels through the Gen-1 map, passes all
five standard anomaly tests. In the Pati--Salam unification it
is known that $Y$ and $N_c$ are linked so that $Y\,N_c$ is
integer; ISPG reaches the same integrality through a different
route (single-scalar cavity labels), without postulating a
larger unifying gauge group.
...
constitute a derivation of the SM hypercharges from pulson
dynamics.
...
\subsection{The number three: geometric, not empirical}
...
\begin{itemize}
  ...
  \item a 3D pulson admits three orthogonal vibration axes;
  \item each axis supports an independent family of
    cavity modes;
  \item therefore there are three generations.
\end{itemize}
...
\end{equation}
ties each generation to one spatial axis. The three-axis
hypothesis is model-dependent (it assumes that the pulson's
axial anisotropy is the source of generation multiplicity), but
its numerical predictions for the charged leptons (Koide,
Sargent, absolute $\tau$) have been verified to sub-percent
accuracy in the companion MASS work (\S\ref{sec:prereq}).
...
The three generations are a structural replica of generation 1
across three orthogonal spatial axes. Anomaly cancellation is
automatic for any number of generations. The question ``why
three'' becomes geometric (3D space) rather than empirical.
...
$m_N=(N/N_e)^{2}\,m_e$ with $N_e=5$ uniformly across all twelve
massive Standard-Model states (three charged leptons, six
quarks, $W$, $Z$, $H$) yields the following compatibility table.
...
\]
i.e.\ $0.04\%$ agreement with the SM identity
$m_W/m_Z=\cos\theta_W$; this is
Eq.~\eqref{eq:cosThetaW} re-evaluated on the full table.
...
accommodates all twelve massive SM states within sub-percent
residuals across the heavy sector. The structural content that
survives parameter counting is the \emph{uniformity}: one
formula, one anchor, one ladder, twelve particles across four
sectors, no Yukawa coupling, no scalar VEV, no sector-specific
coupling constant. Promotion from fit to derivation requires
producing the $N_f$ themselves from cavity dynamics (items E1
and E5 of \S\ref{sec:extension}); the partial 3D-cavity result
for $N_e=5$ and for $N_\tau^{\,2}/N_e^{\,2}$ recorded under E5
...
A & gauge sector $\mathrm{SU}(3)\times\mathrm{SU}(2)\times\mathrm{U}(1)$
  & SU(3) full; EW numerical \\
B/C & Gen-1 fermion quantum numbers $(Q,T_3,Y,N_c)$ & full \\
D & EW breaking without Higgs & full (structural) \\
E & five gauge anomalies                            & full \\
F & three generations as three axes                 & full (model-dep.)\\
\bottomrule
\end{tabular}
\end{center}
...
generation count) is in contradiction with the ISPG pulson +
...
(E6), (ii) a derived pulson-interior mechanism for gauge-boson
...
\begin{itemize}
  \item \derived\ (structurally, from ISPG substrate):
    the \emph{count} of eight confined modes of the
    $D_3$-deformed oscillon; the Mathieu mass law
    $m\propto N^2$ (from the companion MASS paper);
    the linearity of gauge anomalies in $n_{\rm gen}$;
    the integrality of $Y\,N_c$ as a necessary (not
  ...
  \item \modeldep\ (hypothesis):
    ...
    the pulson-interior mechanism for gauge-boson mass.
\end{itemize}
...
(\texttt{.cursor/rules/ISPG\_EightGluon\_Robustness.md}). The present
...
The ratio $N_W/N_Z$ is already derived (\S\ref{sec:phaseA_ew});
the individual values require diagonalizing the vector Laplacian
on the 3D pulson background. A plausible attribution (to be
tested numerically) is $N_W$ to the $\ell=1$ triplet with a
...
structure of $S_m$. A cavity origin would require identifying a
parity-breaking boundary condition or a topological mode that
distinguishes left- from right-handed solutions. This is a
conceptually distinct task from the embedding itself.

\paragraph{E3. CKM and PMNS matrices.}
Under the three-axis hypothesis, the mixing matrices describe
small non-orthogonality of the three oscillation axes. A
principled derivation would map the Wolfenstein parameters to
cavity quantities; at present this is an open target.

\paragraph{E4. $\alpha$ and $\Phi_c$.}
The ISPG Lagrangian contains two dimensionful scales ($\alpha$
for the self-interaction, $\Phi_c$ for the pulson amplitude) that
are currently fit. A first-principles derivation would proceed
via the nonlinear pulson profile at fixed cosmological
background; recent work identifies this as a BVP in the
stationary $\Phi$ equation.
...
the three-axis ansatz to a deduction from the pulson ground
...
is reproduced by the ratio of squared radial wavenumbers of the
corresponding cavity modes to within $0.9\%$. These results fix
the kinematic skeleton of the three-axis ansatz (dimensionality
of the mode space, radial wavenumber ratios) at the
cavity-geometry level.

\deferred\ Extending the partial result to absorb the $0.9\%$
residual requires a Floquet treatment of the periodic pulson:
...
\paragraph{E7. Pulson-interior gauge-boson mass generation.}
A derivation showing that an $\mathrm{SU}(2)\times\mathrm{U}(1)$
gauge field minimally coupled to the pulson acquires a mass
...
dynamical gap in the present paper. Plausible route: expand
the gauge kinetic term around the radially varying pulson
profile $\Phi(r)$ and show that the volume integrals yield the
observed mass pattern when combined with the cavity labels
from E1.
...
\texttt{phase\_a\_gauge\_sector.py}
  & EW boson Mathieu indices; $\cos\theta_W$ ratio test
  & A \\
...
\texttt{gluon\_su3\_closure.py}
  & $\mathrm{SU}(3)$ Lie-algebra closure on the three colour axes;
    $f^{abc}$ matched to textbook to $\leq 2\times 10^{-16}$; Jacobi identity
  & A/E6 \\
\texttt{higgs\_replacement\_unified.py}
  & Unified compatibility table $m=(N/N_e)^2 m_e$ across 12 SM particles
    (leptons, quarks, $W,Z,H$); mean residual $0.85\%$
  & A--D \\
\bottomrule
\end{tabular}
\end{center}
