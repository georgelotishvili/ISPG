...
medium vibrating at a single fundamental frequency
$\nu_0 \approx 1.91 \times 10^{30}$~Hz---the
``note'' of the cosmos. This vibration is energy;
...
(oscillons)---these are particles. Each particle is
a harmonic of $\nu_0$, just as a guitar string
produces overtones $2f, 3f, 4f, \ldots$ The
oscillon, in turn, generates a static gravitational
...
In the ISPG (Isotropic Scalar-Pressure Gravity) framework
...
\begin{itemize}
  \item the medium possesses a single universal vibration frequency
    $\nu_0$;
  \item every \emph{massive} particle is an integer harmonic of
    this frequency; neutrinos are identified as
  ...
  \item the mass formula $m_N = m_1 \times b_N(q)$ (Mathieu
    characteristic value \cite{McLachlan1947,DLMFMathieu}) reproduces all massive
    elementary particle masses to $< 0.3\%$ accuracy
    \cite{PDG2024}
    (muon: $< 0.001\%$; heavy particles with
    reoptimized $N$: $< 0.06\%$);
  \item the electron is the lightest stable particle because
    $N = 1$--$3$ modes lie in the Mathieu instability zone;
...
(ii) the numerical value of $\nu_0$ is fixed by calibration at the
Planck mass (\S\ref{sec:nu0_computation});
...
identified the fundamental frequency $\nu_0$ as the invariant
element required by the very concept of time dilation.  We now
promote this from a postulate to a theorem by proving it from the
...
$M_{\rm Pl}$, $\alpha$, $M$ appearing in $G_2(X)$.  The localized
oscillon solution therefore has proper-time frequency
$\omega_0 = 2\pi\nu_0$ determined solely by these constants and
the substrate's micro-structure---none of which depend on
$\phi_{\rm bg}$.  $\square$
...
at every cosmological epoch.  Every particle is a harmonic of
this one frequency.
}}
\end{center}
\bigskip
...
The classical proof of \S\ref{sec:freq_inv_theorem}
established $\nu_0$ invariance from the structure of the
field equations.  A deeper question remains: \emph{why}
does the medium oscillate at a single, universal frequency?
...
\medskip
\textbf{Quantization of the pulson field.}
...
\end{equation}
where $n_0 = N_0/V$ is the pulson number density.
...
\end{tabular}
\end{center}
The ``one pulson everywhere'' of the intuitive picture
...
If $\nu_0$ exists, it is the \emph{common multiple} of known
particle frequencies. Every particle vibrates at its Compton
frequency $\nu = mc^2/h$ (de~Broglie, 1924; M\"uller, 2013). If
these frequencies are harmonics of a single fundamental
(Section~\ref{sec:harmonic}), then $\nu_0$ must be the least
common multiple (or a multiple thereof) of these frequencies.

Concretely: if particle mass ratios are rational numbers (or very
good rational approximations), $\nu_0$ is computationally
determinable from the known mass spectrum.
...
If fundamental frequency invariance is accepted as a principle,
then:
\begin{center}
...
In the previous section a single fundamental frequency $\nu_0$ was
postulated to be invariant. Yet nature contains different particles
with different masses---electrons, muons, quarks, bosons. If mass
and frequency are related ($E = mc^2 = h\nu$), different masses
imply different frequencies. How is this compatible with a single
fundamental frequency?
...
$n = \pm 1$), antiparticles (opposite winding), bosons ($n = 0$).
The topological classification determines the particle's
\emph{type}. The harmonic resonance principle complements this:
...
\begin{enumerate}
  ...
  \item[\checkmark] The mass spectrum has been compared with
    experiment---\textbf{all} known elementary particles fit the
    formula to $< 0.3\%$; the muon to $< 0.001\%$.
...
\emph{electron's special status as the lightest stable particle}---
all from the resonant structure of a single medium.

%% ==========================================================
\section{Determination of the Universal Frequency $\nu_0$}
\label{sec:nu0_computation}
%% ==========================================================

Goal: compute $\nu_0$---the fundamental vibration frequency of the
spatial medium, from whose harmonics every particle arises.
...
\subsection{Computing $\nu_0$ from the Planck Scale}

The fundamental mode ($r = 1$) corresponds to the
...
\textit{Note.}---This classical estimate
...
solely as a pedagogical stepping stone.
...
\subsection{Particle Harmonic Map}

$r = 1$ fundamental mode $=$ Planck mass.
...
In the ISPG framework this means: particles are
\textbf{extremely high harmonics} of the medium---very slow
bouncing between very rapidly vibrating plates. Modes close to
...
(discrete spectrum, hierarchy) is the same; only the
labeling and the numerical value of the fundamental
frequency change.
...
\begin{equation}
  a = \left(\frac{E}{\hbar\nu_0}\right)^2
  = \left(\frac{mc^2}{\hbar\nu_0}\right)^2
  = \left(\frac{m}{m_P}\cdot\pi\right)^2,
  \qquad
  q \approx 1 + \delta q,
...
\begin{equation}
  m_n = \frac{\hbar\nu_0}{\pi c^2}\,\sqrt{a_n(q)}
  = \frac{m_P}{\pi}\,\sqrt{a_n(q)}.
  \label{eq:mass_from_mathieu}
\end{equation}

After determining $q$ exactly, the Mathieu characteristic values
$a_n(q)$ yield the \textbf{complete particle mass spectrum},
...
\medskip\normalsize
This is the frequency at which space itself ``breathes.'' Every
$\pi\,t_P = 1.7 \times 10^{-43}$ seconds the medium completes
one full vibration. Every particle is a harmonic of this vibration.
}}
\end{center}

\begin{itemize}
  \item \textbf{Particle $=$ sub-harmonic of the medium.}
    ...
    Faster bouncing $=$ greater mass.
  \item \textbf{Planck limit.} $r = 1$---bouncing on every
    vibration---yields
    $m_P = 1.22 \times 10^{19}$~GeV.
    A heavier oscillon is physically impossible.
  \item \textbf{Wavelength $=$
    $\pi\,\ell_P \approx 5 \times 10^{-35}$~m.}
    This is the medium's ``granularity'' scale.
    At shorter distances, ``space'' is discrete.
  \item \textbf{Mass $=$ frequency.} For every particle:
    $m = \hbar\nu_0 / (\pi r c^2) = m_P / r$.
    Mass is directly a function of $\nu_0$ and $r$.
\end{itemize}

\subsection{Completed Program}

The numerical value of $\nu_0$
\eqref{eq:nu0_numerical} relies on the assumption that
$r = 1$ corresponds to the Planck mass. The quantum
model performed a recalibration.
The following program has been completed:
\begin{enumerate}
  \item[\checkmark] \textbf{Derivation of $q$ from ISPG
    $G_2(X)$ extension}---
    $q = 1$ at leading order (flat-space
    self-consistency), $q = 1 + \beta\Phi_0^2
    = 1.853$ with bi-conformal metric backreaction,
    gradient, and harmonic corrections
    (\S\ref{sec:q_derivation}).
...
\medskip
\textbf{Automatic satisfaction of the Koide formula.}
...
This means: the Koide formula, an unexplained empirical relation
since 1981, \textbf{automatically} follows from the integers
$(5, 72, 295)$ in the two-plate model.  A rigorous treatment
from the full three-dimensional cavity spectrum---with
Rank-1 perturbation theorem and Hartree corrections
reproducing $Q = 2/3$ to $0.009\%$ accuracy---is given in
ISPG\_Quantum, Sec.~7.2.
...
(unstable for $N = 1$--$3$, marginal for $N = 4$,
stable for $N \ge 5$), which independently
explains why the electron is the lightest
charged particle;
(iii)~the residual errors ($< 0.3\%$ for
precisely measured particles) are systematic,
not random, indicating that they encode the
Mathieu eigenvalue corrections $b_N(q)$.
The programme becomes fully predictive once
$q$ is derived from the lattice simulation of
$G_2(X)$ (future work, \S\ref{sec:future}).
...
\medskip
\textbf{Assessment:} For precisely measured particles
($\mu$, $\tau$, $b$, $W$, $Z$, $H$, $t$),
the error is $< 0.3\%$---twelve masses reduce to
\textbf{one scale ($m_1$) plus one integer per
particle}.  The Standard Model requires twelve
independent Yukawa couplings for the same data.
...
The $N = 1$ mode is likely unstable (in the Mathieu
instability zone); the lightest stable mode would be $N = 2$
or $N = 3$:
\begin{alignat}{2}
  m_{N=2} &= 4\,m_1 = 81.8\;\text{keV}
  &\qquad& (\text{sterile neutrino?}),\\
...
\textbf{This is the definitive value of $\nu_0$ used
throughout this paper.}  It supersedes the classical
...
yields \textbf{significantly better} mass ratios (integers!).
...
\textbf{determined by} the vibration amplitude $\Phi_0$,
which shares a common origin with $m_e$ and $m_P$
through $\nu_0$ and $G_2(X)$
(\S\ref{sec:higgs}).
...
\medskip
\textbf{Determining $q$ from the muon mass.}
...
At this $q$ the muon mass is reproduced \textbf{exactly}
($< 0.001\%$ error).

\medskip
\textbf{Stability analysis: why $N_e = 5$?}

Instability zone widths at $q = 1.853$:
...
\textbf{The electron ($N = 5$) is the lightest stable particle
because $N = 1$--$3$ lie in the Mathieu instability zone,
while $N = 4$ is marginal.}
}}
\end{center}
...
In the previous section $q = 1.853$ was determined from the muon
mass (by fitting). We now show that the \textbf{leading-order
value $q = 1$ follows from first principles}, and identify three
calculable correction channels (metric backreaction, spatial
gradients, higher harmonics) that account for the remainder
$\delta q = 0.853$---all without introducing new fields or
breaking the shift symmetry
$\phi \to \phi + \mathrm{const}$ that protects gravity's
long-range nature. The exact value of $\delta q$ awaits lattice
...
\begin{equation}
  ...
  {3\,\alpha\,\omega_0^2},
  \qquad
  \beta\,\Phi_0^2 = q - 1 = 0.853.
  \label{eq:amplitude_constraint}
\end{equation}

\textbf{Prediction:} the peak dimensionless
oscillon amplitude satisfies
$\Phi_0 = \sqrt{0.853/\beta}$.
For $\beta = 1$ (all corrections comparable):
$\Phi_0 \approx 0.924$, deep in the nonlinear
regime where both the $G_2(X)$ and metric effects
are $\mathcal{O}(1)$---precisely the oscillon
self-trapping condition.
...
of order $\Phi_0^2$, shifting $q$ to its
physical value $1.853$. The coefficient $\beta$
and the oscillon amplitude $\Phi_0$ are
...
\end{equation}
giving a total $q = 1 + 0.907 = 1.907$, within $2.9\%$ of
the fitted value $q = 1.853$. The dominant correction
(metric backreaction, $73\%$ of $\delta q$) is computed
exactly; the remainder comes from volume-averaged gradient
and order-of-magnitude harmonic estimates. At $q = 1.907$
...
the precision of the semi-analytical gradient and harmonic
formulas; it will be resolved by full lattice PDE evolution
of the oscillon profile (future work).
The simulation code is provided as
\texttt{space/python/ispg\_oscillon\_simulation.py}.

\medskip
\textbf{Step 7: Why $q$ is universal.}

The parameter $q$ depends on the background oscillon
properties ($\varepsilon$), not on the perturbation mode
number $N$. This universality follows directly from the
...
acts identically on all length scales, so no mode-dependent
$q$ can arise.

This is confirmed numerically
(\S\ref{sec:mathieu_numerical}): a single constant
$q = 1.853$ reproduces all lepton masses and, with
re-optimized $N$ assignments, all heavy particles to
$< 0.06\%$.
...
metric backreaction + gradients + harmonics
($\sim\beta\Phi_0^2$, lattice-determinable).
}}
\end{center}
...
\textbf{Result} & \textbf{Accuracy} & \textbf{Ref.}
  \\[2pt]\hline\\[-6pt]
All particle masses: $m_N = m_1 b_N(q)$
  & $< 0.3\%$ & \S\ref{sec:numerical_results} \\
Muon mass ($q = 1.853$)
  & $< 0.001\%$ & \S\ref{sec:mathieu_numerical} \\
Heavy particles (reoptimized $N$)
  & $< 0.06\%$ & \S\ref{sec:mathieu_numerical} \\
Koide formula (inherited from individually-fit lepton masses;
\emph{not} an independent derivation --- see QUANTUM Sec.~7 for
the independent rank-1 derivation)
  & $0.028\%$ & \S\ref{sec:mathieu_numerical} \\
Electron---lightest stable particle
  & Mathieu & \S\ref{sec:mathieu_numerical} \\
$q = 1$ from $G_2(X)$ self-consistency (leading order)
  & analytic & \S\ref{sec:q_derivation} \\
$q = 1 + \beta\Phi_0^2 = 1.853$ (bi-conformal corrections)
  & structural & \S\ref{sec:q_derivation} \\
$q$ is universal (sector-specific $q$ excluded)
  & numeric & \S\ref{sec:mathieu_numerical} \\
...
$q = 1.853$, the integer $N$ associated with each particle
...
(at $q = 1.853$, $\Delta N \approx 1.5N/1000$):
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}lrrrr@{}}
...
$\tau$& 295  & 1775.3    & 1776.9    & $0.09\%$ \\
$c$   & 250  & 1274      & 1270      & $0.3\%$ \\
$b$   & 453  & 4183      & 4180      & $0.06\%$ \\
$W$   & 1986 & 80389     & 80379     & $0.01\%$ \\
$Z$   & 2115 & 91172     & 91188     & $0.02\%$ \\
$H$   & 2477 & 125052    & 125100    & $0.04\%$ \\
$t$   & 2911 & 172713    & 172760    & $0.03\%$ \\
\end{tabular}
\end{center}
...
\begin{enumerate}
  \item \textbf{$N = 4$ resonance:
    ...
    The Mathieu instability zone width is $0.01$---very
    short-lived
    ($\tau \lesssim 10^{-20}$~s), but not instantaneous.
    Search areas:
    \begin{itemize}
      \item keV-range sterile neutrino
        (X-ray astronomy, $\sim 165$~keV
        line from $m_4/2$).
      \item Dark matter candidate
        ($\Omega_{\mathrm{DM}}h^2$ recalculation).
      \item Laboratory search: beta-spectrum kink
        at $329$~keV (KATRIN-type experiment).
    \end{itemize}
  ...
  \item \textbf{Light quark mass verification.}
    The model predicts:
    $m_u = 2.04$~MeV, $m_d = 4.59$~MeV.
    Both are within PDG uncertainties,
    but future lattice QCD calculations can verify them.

  \item \textbf{Cosmological frequency
    $\nu_0 = 1.91 \times 10^{30}$~Hz.}
    This is the medium's fundamental vibration frequency.
    Indirect test:
    \begin{itemize}
  ...
  \item \textbf{k-Oscillon in $G_2(X)$:
    ...
    \begin{itemize}
      ...
      \item Corrections:
        ...
        coupling---all must sum to $0.853$.
...
fixed by fitting the electron mass
(\S\ref{sec:numerical_results}).
...
Spin-1/2 $=$ vortex winding number $n = \pm 1$,
$J = n\hbar/2$; Fermi--Dirac statistics from
Berry phase $= \pm\pi$; Pauli exclusion from
vortex topology (ISPG\_Quantum, Sec.~10).\\
...
Higgs VEV $=$ vibration amplitude $\Phi_0$;
Higgs boson $=$ amplitude fluctuation
(Sec.~\ref{sec:higgs}); formal derivation
$v(\varphi) = v_0\,e^{\varphi/2}$ with full
electroweak spectrum in ISPG\_Quantum, Sec.~8.
}}
\end{center}
...
successfully reproduces all \emph{massive}
...
\begin{itemize}
  ...
  \item Berry phase $\gamma = \pm\pi$
    under exchange $\Rightarrow$
    Fermi--Dirac statistics
    (Eq.~(10.12) of ISPG\_Quantum);
  \item opposite winding $\Rightarrow$
    antiparticle
    (Sec.~10.2 of ISPG\_Quantum).
\end{itemize}

The neutrino therefore inherits all fermionic
properties from the same topological mechanism
that governs electrons and quarks.  The
...
an \textbf{antineutrino}---a vortex of opposite
winding ($n = -1$).  Simultaneously, the change
...
The formal derivation of the Higgs mechanism
...
the full electroweak gauge boson spectrum
($W^{\pm}$, $Z$, $\gamma$), and predicting
...
This theme identifies the Higgs mechanism
unambiguously.

\textit{The Higgs field is the vibration
amplitude $\Phi_0$.}---The $\nu_0$-string
...
This is \emph{precisely} what the Standard
Model's Higgs vacuum expectation value $v$
does: with $v = 0$ all fermions and gauge
bosons are massless; with $v \neq 0$ they
acquire mass proportional to their coupling.
...
\textit{Spontaneous symmetry breaking.}---The
$G_2(X)$ action determines the equilibrium
amplitude $\Phi_0$
(Eq.~\ref{eq:amplitude_constraint}).
The linear term
...
$\omega_0$.  This is the
\textbf{Mexican-hat potential}---not postulated,
but derived from the $G_2(X)$ action.
...
\begin{equation}
  ...
  \;\propto\; m_N,
\end{equation}
reproducing the Standard Model result that the
Higgs couples to each particle in proportion to
its mass.
...
$\to$ \textbf{massive Higgs boson}.
Fluctuations of $\theta_i$ cost no energy
(flat direction of the hat)
$\to$ \textbf{three massless Goldstone bosons}.
These Goldstone modes are ``eaten'' by the
$W^\pm$ and $Z^0$ through the
torsion--transverse coupling
(\S\ref{sec:WZ_bosons}), giving them
their observed masses ($m_W \approx 80$~GeV,
$m_Z \approx 91$~GeV).  The number of
Goldstone bosons (three) matches the three
...
close to $m_W = 80$~GeV
(\S\ref{sec:mathieu_numerical}).
In the ISPG picture this is not a coincidence:
the electroweak scale $v_{\mathrm{EW}}$ is
determined by $\Phi_0$, which is fixed by
$\omega_0 = 2\pi\nu_0$ and the $G_2(X)$
parameters
...
\textit{Future work.}---Deriving the numerical
value $m_H = 125$~GeV from
$V_{\mathrm{eff}}''(\Phi_0)$ requires
$\alpha/M^2$, which will be determined by
the lattice simulation of k-oscillons
(Eq.~\ref{eq:amplitude_constraint}).
Once $\alpha$ and $M$ are fixed independently,
$m_H$ becomes a first-principles prediction.
The precise mapping between $\Phi_0$ and
$v_{\mathrm{EW}} = 246$~GeV through the
bi-conformal normalisation conventions is
a natural part of that calculation.
...
(strong).\\[2pt]
11.\ Higgs VEV $=$ vibration amplitude $\Phi_0$;
Higgs boson $=$ amplitude fluctuation $h(x,t)$;
Mexican hat $=$ $G_2(X)$ effective potential
(\S\ref{sec:higgs}).\\[6pt]
\textit{Established in the companion paper
(ISPG\_Quantum):}\\[2pt]
12.\ Spin-1/2 $=$ vortex winding number,
$J = n\hbar/2$; Fermi--Dirac statistics from
Berry phase (Sec.~10).\\[2pt]
13.\ Higgs mechanism $=$ conformal symmetry
breaking, $v(\varphi) = v_0\,e^{\varphi/2}$;
full electroweak spectrum recovered
(Sec.~8).\\[2pt]
14.\ Koide formula $Q = 2/3$ from 3D cavity
eigenvalues, $0.009\%$ accuracy (Sec.~7.2).\\[2pt]
...
\begin{itemize}
  \item[\checkmark] \textbf{Spin-1/2 and
    Fermi--Dirac statistics}: $J = n\hbar/2$ from
    vortex winding; Berry phase $= \pm\pi$
    for $n = \pm 1$ exchange; Pauli exclusion from
    vortex topology (Sec.~10).
  \item[\checkmark] \textbf{Higgs mechanism}:
    $v(\varphi) = v_0\,e^{\varphi/2}$ as
    conformal symmetry breaking; full electroweak
    gauge boson spectrum recovered; gravitational
    correction $\delta m_H^2 \sim 10^{-34}\,m_H^2$
    predicted (Sec.~8).
  \item[\checkmark] \textbf{Koide formula}:
    $Q = 2/3$ from 3D cavity eigenvalues with
    Rank-1 perturbation theorem and Hartree
    corrections, $0.009\%$ accuracy (Sec.~7.2).
...
\begin{itemize}
  ...
  \item \textbf{Higgs mass from first principles}:
    derive $m_H = 125$~GeV from
    $V_{\mathrm{eff}}''(\Phi_0)$ using $\alpha$
    and $M$ from the lattice simulation
    (\S\ref{sec:higgs}).
