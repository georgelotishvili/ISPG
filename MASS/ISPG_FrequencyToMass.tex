...
\begin{itemize}
  ...
  \item the integer-harmonic structure of the scalar sector
    ...
    ($\ell \geq 2$) modes to propagate selects $d=3$ as the
    minimal spatial dimension, while exact $d=3$ rather than
    $d>3$ requires the minimality/empirical input stated in
    \S\ref{sec:dimensions}.
\end{itemize}
...
\medskip
\textbf{Measurable-existence guardrail.}
The measurable-existence identification gives an operational reading of
...
this identification does not derive $\nu_0$, does not derive
$1.91\times 10^{30}$~Hz, does not prove the mass ladder, and does
not close the Mathieu $q$ derivation.
The notation $\mathcal N_{\rm act}$ used below denotes the matter-standard
side of this measurable-existence identification: the effective amount of
interaction-active resonant participation carried by a localized oscillon or
measuring standard.  It is distinct from the condensate occupation $N_0$ and
from the density $n_0=N_0/V$.  The latter describe the background condensate;
$\mathcal N_{\rm act}$ describes the operational content of a resonant
standard and is normalized through ratios such as
$\mathcal N_{\rm act}(\phi)/\mathcal N_{\rm act}(0)$.
This is an identification, not a synonym: in the local/static sector the
effective mass $m_{\rm eff}$, the matter-standard length $L$, and the clock
rate are read as projections of the same active measurable content, with
$m_{\rm eff}$ playing the role of the asymptotic mass/energy projection
rather than an independent renaming of $\mathcal N_{\rm act}$.
...
\begin{enumerate}
  ...
  \item Rod postulate: the oscillon's local size $\ell_0$ is
    ...
    In the measurable-existence identification we may also write
    ...
    \end{equation}
    This is an operational identification of $\mathcal N_{\rm act}$ through
    the two-plate matter-standard ratio, not an independent number-density
    count of medium nodes, not a three-dimensional volume count, and not a
    new observable.
...
impulse rate.  Equivalently, it is the asymptotic mass/energy projection of
the same operational active measurable content that controls the effective
matter-standard extension in \eqref{eq:Nact_two_plate}.  By normalization:
\begin{equation}
...
non-integer, highly incommensurate, or very complex.
...
suppressed torsional rigidity supplies a partial mass
suppression; reaching the sub-eV scale additionally requires
the open trapping factor $\eta_T$ derived in
\S\ref{sec:neutrinos}.
...
Gravitational phase/decoherence channel $=$
leading coherent rate
$\Gamma_{\varphi} = 2m_0|\Delta\Phi_N|/h$;
the $\sim 0.5$~Hz eV-photon coefficient is formal and
short-flight limited, while irreversible decoherence is
conditional on the tail/noise correlator
(ISPG\_Quantum, Sec.~9).
\\
...
The polarisation-rotation mode supplies a
structural candidate for several distinctive
neutrino properties that the Standard Model treats
...
\begin{enumerate}
  \item \textbf{Intrinsic chirality.}\;
    ...
    number.  This gives a candidate geometric
    route to the observed chiral bias of weak
    neutrino interactions.  For massive neutrinos
    the chirality/helicity distinction remains
    essential, and the precise map to the SM
    left-handed weak current is part of the open
    weak-sector derivation.
  ...
  \item \textbf{Extreme lightness.}\;
    ...
    ratio.  This supplies only a partial
    suppression; placing neutrino masses in the
    sub-eV range also requires the weak-trapping
    factor $\eta_T$ in Eq.~\eqref{eq:trapping_fraction},
    whose first-principles value remains a future
    calculation.
  ...
  \item \textbf{Extremely weak coupling.}\;
    ...
    This is a candidate \textbf{weak-sector}
    coupling vocabulary: a nonlinear coupling
    between the polarisation-rotation sector and
    the scalar/vector sectors.  The full
    V--A current, $G_F$, lepton-number rules,
    beta spectra, and parity-violation amplitudes
    remain part of the open weak-interaction
    derivation.
  ...
  \item \textbf{Flavour oscillations.}\;
    ...
    The mismatch is a candidate route to the PMNS
    mixing matrix and flavour oscillation.  Deriving
    the oscillation Hamiltonian, mass-squared
    splittings, mixing angles, and CP phase remains
    future work.
...
where $\lambda_0 = c/\nu_0
= \pi\ell_P \approx 5\times 10^{-35}$~m
in the historical carrier-wavelength convention
(superseded for the torsional suppression by
$\lambda_{\mathrm{osc}}$ in Eq.~\eqref{eq:Rperp_corrected}
below)
and $R_c = \hbar/(m_e c)
...
\sim 1/(2\pi)^2 \approx 0.025$.  Acting on the
electron scale this gives
$m_e/(2\pi)^2 \simeq 13~\mathrm{keV}$, i.e.\ about
1.6 orders of magnitude of suppression, not the
full $m_\nu/m_e \sim 10^{-7}$ hierarchy.  Reaching
the sub-eV regime requires the trapping fraction
to provide the dominant remaining suppression,
typically $\eta_T \sim m_\nu/(13~\mathrm{keV})
\sim 10^{-6}$ for the observed neutrino scale.
This remaining suppression arises because the torsional mode,
...
is a candidate route to an exact chirality label
in the massless limit.  For massive neutrinos,
chirality and helicity must be distinguished: the
small mass term $\Omega_T$ in
Eq.~\eqref{eq:torsion_equation} would allow mixing
between $n = +1$ and $n = -1$ with the standard
$m_\nu/E$ helicity-flip suppression.  A full
Weyl/Dirac spinor derivation remains part of the
open weak-sector programme.
...
In this candidate picture, the $W$ boson is the
\textbf{massive mediator of the torsion--transverse
coupling}: a nonlinear excitation that temporarily
converts polarisation-rotation energy into transverse
energy and vice versa.  Its large mass
($m_W \approx 80$~GeV) reflects the suppressed
coupling between these sectors.  The $Z$ boson
is treated as the neutral analogue: a
...
\begin{itemize}
  ...
  \item Spin-$\frac{1}{2}$, Fermi--Dirac
    statistics, and chirality have a candidate
    structural route through vortex topology
    (\S\ref{sec:neutrino_vortex};
    ISPG\_Quantum, Sec.~10); the full
    Fock-space/spinor and massive-neutrino
    chirality derivations remain open.
  \item The two-component circular-polarisation
    space coupled by $G_2(X)$ supplies a candidate
    route toward the weak doublet structure
    (\S\ref{sec:WZ_bosons}).
  \item The promotion of this candidate SU(2)-like
    structure to the SM local $\mathrm{SU}(2)_L$
    gauge theory via the spatial dependence of
    $\Phi_0(r)$ remains a future formal
    Yang--Mills derivation.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Neutrino masses.}\;
    ...
    together with the relevant boundary/profile data,
    requires a dedicated lattice computation and is
    the topic of a separate work.  Before that
    calculation the three neutrino masses are not
    independently predicted; after it, the parametric
    form can be tested without adding an extra
    phenomenological mass factor.
...
and the structural confinement mechanism is developed below.
...
\medskip
Two statuses are kept distinct in this section:
(i) the imported standard photon field $A_\mu$
used for present electromagnetic propagation
calculations, and (ii) the future programme of
deriving an emergent helicoidal/transverse-vector
medium mode that would reproduce that photon.
Only the first status is used below.
...
The $\nu_0$-background suggests a dispersive
...
candidate for a non-propagating or evanescent
exterior mode.  The inharmonicity criterion
therefore supplies a structural route to gluon
confinement by \emph{frequency mismatch}.  A
complete proof of the band gap and decay length
requires the exterior $G_2(X)$ dispersion relation
and boundary matching problem to be solved.
...
\begin{itemize}
  ...
  \item \textbf{Tensor perturbations}
    ...
    \end{equation}
    a non-integer ratio.  The tensor modes form
...
This is the mathematical origin of the
...
that are \emph{incommensurate} with the integer
harmonic tower:
\begin{equation}
  \boxed{
...
is expected to couple poorly to coherent
propagation in the $\nu_0$-background; whether it
decays evanescently, and with what decay length,
is the exterior band-gap calculation left for
future work.
...
$j_{\ell \geq 2,\,n}/\pi \notin \mathbb{Z}$)
is a theorem of Bessel function theory.  The
identification of this asymmetry with physical
confinement is a structural mechanism: it becomes
a derivation only after the exterior propagation
problem for the full medium is closed.
...
\begin{enumerate}
  \item \textbf{Confinement.}\;
    In this candidate mechanism, a single gluon
    cannot exist in free space because the
    $\nu_0$-background does not support coherent
    propagation of the inharmonic tensor-cavity
    mode.
    Photons \emph{can} propagate freely because,
    in the present treatment, they are described
    as the imported $U(1)_{\mathrm{EM}}$ gauge
    mode $A_\mu$ rather than as localized resonant
    excitations; a direct medium-mode
    identification (helicoidal/Kelvin/vector-mode
    candidate) remains future work.
  ...
  \item \textbf{Flux tube and linear potential.}\;
    ...
    \begin{equation}\label{eq:flux_tube}
      ...
      R_{\mathrm{core}}^2,
    \end{equation}
    giving a route to the linear confining
    potential used in lattice QCD.  If this
    flux-tube picture is dynamically realized,
    then when $V(L)$ exceeds $2m_q c^2$, the tube
    breaks and a new quark--antiquark pair
    materializes---the analogue of QCD string
    breaking.
\end{enumerate}
...
directly with lattice-QCD glueball benchmarks
...
\begin{equation}\label{eq:fundamental_ratio}
  ...
  \qquad (\Delta = 0.66\%),
\end{equation}
provides a suggestive numerical consistency check.

\textit{Scope of the ratio comparison.}---%
...
in that limited sense the four-state comparison
uses no additional tunable coefficient.  It is not
a standalone experimental confirmation of QCD
glueballs.  Two caveats must be stated
honestly.  (i)~The
...
to set the absolute scale.  The internal
glueball-ratio pattern is therefore a
coefficient-free lattice-benchmark comparison
under the chosen assignments, while the proton-anchor
agreement in
...
$\varepsilon_{\text{pert}} \approx 0.5$, so the
required value lies in a genuinely nonperturbative
regime.  An exploratory script
\texttt{python/gluon\_epsilon.py} was intended to
estimate this scale by comparing circular and
$D_3$-deformed two-dimensional cavities, but its
current circular-cavity benchmark is not calibrated:
it does not reproduce the expected low-lying Bessel
spectrum before the triangular deformation is added.
Consequently its previously quoted
$\varepsilon_{\text{2D}}$ and dressed
$\varepsilon_{\text{3D}}$ values are not used as
evidence here.  The reliable quantitative target is
the analytic requirement
$\varepsilon \approx \Delta/0.492 \approx 13$;
deriving whether the real 3D oscillon reaches this
target requires a corrected finite-element or
spectral eigenproblem.

\textit{Non-independence note.}---%
If a future 2D-to-3D dressing calculation uses
$j_{2,1}/\pi = 1.835$, it will share the same
Bessel-zero input that appears in
Eq.~\eqref{eq:fundamental_ratio}.  Such a result
should therefore be counted as a single structural
thread, not as an independent second numerical
prediction, until the full 3D eigenproblem is
solved directly.
...
This 3D finite-element calculation will determine
the required nonperturbative splitting directly
and test whether the eight surviving modes'
coupling algebra reproduces SU(3).  If so, the
colour gauge group would emerge geometrically
from the oscillon's $D_3$ cavity structure.
...
\begin{itemize}
  ...
  \item \textbf{Neutrinos}
    ...
    with $n = \pm 1$)
    $\to$ candidate chirality route and
    $m_\nu \ll m_e$ after the open $\eta_T$
    suppression; the weak-sector coupling is
    modeled as nonlinear coupling between
    torsional and other modes.
...
\item \textbf{Third dimension:}
  ...
  These are the \textbf{gluon-candidate tensor
  modes} discussed in Section~\ref{sec:gluon}.
\end{enumerate}
...
\begin{equation}\label{eq:dim_chain}
  ...
  \underbrace{2\text{D}}_{\text{membrane-like transverse sector}}
  \;\xrightarrow{\;\text{tensor vibration}\;}
  \underbrace{3\text{D}}_{\text{cavity}\;(\ell \geq 2\;\text{admissible})}
  }
\end{equation}

The present construction uses the angular sphere
$S^2$ of ordinary three-dimensional space, with
coordinates $(\theta,\varphi)$.  A hypothetical
$d$-dimensional construction would instead use
$S^{d-1}$, and the angular argument alone does
not exclude $d>3$.  Restricting the construction
to exactly $d=3$ therefore requires the
minimality/empirical input stated below, or a
future stability/graph-reconstruction argument.

\textit{Why exactly $3$ and not $2$ or $4$?}---
A purely $(2+1)$-dimensional universe has no
local propagating spin-2/tensor degrees of
freedom in the standard gravitational
degree-count; the transverse-traceless
polarizations vanish at two spatial dimensions.
This is a gauge/dynamical obstruction, not a
harmonic-existence obstruction: harmonics on
$S^1$ exist for arbitrary integer winding.
Three spatial dimensions is therefore the
\textbf{minimum} in which both the scalar sector
and propagating tensor degrees of freedom coexist:
...
support tensor vibrations.  In the present
interpretive picture, the $\ell=2$ tensor
sector---and, in the strong-sector application,
the gluon-candidate sector of
Section~\ref{sec:gluon}---is the structure that
motivates the third spatial dimension.  The full
gluon/confinement dynamics, however, remain open
until the exterior band-gap problem and the
dynamic $\mathrm{SU}(3)_c$ vertices are derived.
The confinement criterion then receives a second
interpretation: it is the mismatch between a
2D membrane-like vibration and a 1D string
...
integer for which the propagating transverse-
traceless spin-2 sector is available in the
usual gravitational degree count.  Larger
spatial dimensions ($d=4, 5, \ldots$) also
support propagating tensor modes and therefore
pass the same structural test.  The argument above does not
...
\textit{Step~2: entanglement area as a route to
geometric area (Ryu--Takayanagi
\cite{RyuTakayanagi2006,NishiokaTakayanagi2009}).}---In holographic
...
This is a guide rather than a theorem already
proved for an arbitrary $G_2(X)$ scalar vacuum:
the ISPG-specific tensor-network/MERA
reconstruction of distances, angles and curvature
from $|\Omega\rangle_\phi$ remains a future
calculation.  The ER\,=\,EPR conjecture
\cite{MaldacenaSusskind2013} is therefore used
here as geometric intuition, not as a completed
derivation.
...
satisfies three consistency conditions.  We check
each as scaffolding for the future reconstruction:
...
\textit{The reconstruction chain (schematic).}---Steps
...
\begin{itemize}
  \item \textbf{Symmetric oscillon}
    ...
    In the ideal fundamental-mode limit it has no
    transverse dipole and is a candidate
    electrically neutral mode.
    $\to$ \textbf{candidate electrically neutral.}
  ...
  \item \textbf{Asymmetric oscillon}
    ...
    is the candidate geometric seed for coupling
    to transverse waves.
    $\to$ \textbf{candidate electrically charged.}
\end{itemize}

This is a charge-like structural assignment, not yet a
derivation of the electromagnetic charge operator.  A
Noether current, an emergent gauge field $A_\mu$, the
Maxwell kinetic term, universal coupling to matter, and the
normalisation of the elementary charge $e$ remain part of
the gauge-completion programme.
...
U(1)-like transverse-angle symmetry}.  Promoting this
global symmetry to a local electromagnetic
$U(1)_{\mathrm{EM}}$ gauge symmetry---that is, deriving the
photon $A_\mu$ and Maxwell dynamics from the medium---is the
gauge-completion programme and is not carried out here.

\textit{Consequences.}---
The consequences below are qualitative pictures that follow
from the candidate transverse-asymmetry route.  They are not
derivations of the Coulomb $1/r^2$ law, the Lorentz force,
linear superposition, or the exact charge-sign convention.
\begin{enumerate}
  ...
  \item \textbf{Ideal zero-dipole modes are
    photon-inactive at this level:}\;
    a fundamental symmetric ($\ell = 0$) mode with
    no transverse dipole would not couple through
    this candidate channel.  Neutral composites,
    atoms, magnetic moments, polarizabilities, and
    electromagnetic form factors are outside this
  ...
  \item \textbf{Fractional charges (quarks):}\;
    ...
    (see the charge programme below).
\end{enumerate}
...
force is conjectured to follow from the analogous
Bernoulli mechanism operating in the \emph{transverse}
sector.  At this stage this is a qualitative candidate
picture, not a derivation of Maxwell electrodynamics or
the Coulomb/Lorentz force law:
\begin{itemize}
...
\item \textbf{Fine-structure constant.}\;
  $\alpha_{\mathrm{EM}}$ should emerge as the ratio of the
  ...
  \begin{equation}\label{eq:alpha_target}
    ...
    \frac{1}{137.036}\,.
  \end{equation}
  Current exploratory scans in
  \texttt{SM/python/scan\_alpha\_*.py} do not yet obtain
  the observed value; deriving the dipole-to-monopole
  amplitude ratio that yields
  $\alpha_{\mathrm{EM}}\simeq 1/137$ remains future work.
...
weak-sector candidate $=$ nonlinear
torsion--transverse coupling; parity violation
has a candidate geometric source in torsional
chirality
...
(Sec.~6).\\[2pt]
17.\ Gravitational phase/decoherence:
$\Gamma_{\varphi} = 2m_0|\Delta\Phi_N|/h$
as the leading coherent rate; the eV-photon
$\sim 0.5$~Hz coefficient is short-flight limited,
and irreversible decoherence is conditional on the
tail/noise correlator (Sec.~9).
}}
\end{center}
...
from $j_0(x) = \sin(x)/x$ to a
lower-bound/minimality route to the observed
three spatial dimensions and to a structural
confinement criterion for tensor modes.  The
quantitative programme---proving that the
one-dimensional radial/Mathieu mass ladder and
the full 3D cavity dynamics reduce to one
another, with an explicit $N\leftrightarrow
(n,\ell)$ map and the observed 3+1-dimensional
spacetime---is a goal for future work.
...
\begin{itemize}
  ...
  \item[\checkmark] Glueball mass-ratio pattern
    from Bessel-function zeros: $\leq 2\%$ for
    four lattice-benchmark assignments, with no
    additional coefficient once the assignments are
    chosen; proton anchoring remains an external
    scale check
    (\S\ref{sec:gluon}).
...
\begin{itemize}
  ...
  \item[$\circ$] \textbf{Gravitational
    phase/decoherence}: $\Gamma_{\varphi}
    = 2m_0|\Delta\Phi_N|/h$ is the leading coherent
    rate; the eV-photon $\sim 0.5$~Hz coefficient is
    short-flight limited, and irreversible decoherence
    is conditional on the tail/noise correlator
    (Sec.~9).
...
\begin{itemize}
  ...
  \item \textbf{Electric charge and
    $\alpha_{\mathrm{EM}} \approx 1/137$}: attempt to derive
    from the dipole-to-monopole amplitude ratio; test whether
    $D_3$ phase locking can reproduce quark charges
    $+2/3$, $-1/3$
    (\S\ref{sec:charge}).
...
\phantomsection
\section*{References}
\input{../MAIN/ISPG_References.tex}
