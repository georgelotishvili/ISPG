...
(vii)~gravitational birefringence from one-loop vacuum polarization,
with differential phase $\Delta\Phi_{\mathrm{biref}} \sim 10^{-4}$~rad
for gamma-rays near stellar-mass black holes, detectable by AMEGO
and e-ASTROGAM; (viii)~anisotropic quantum tunneling enhanced toward
...
and testable by ESPRESSO and ELT-ANDES; (x)~a numerical derivation of the Koide formula $Q = 2/3$ for
lepton masses from the cavity spectrum of the exponential
nonlinearity, reproducing $Q$ to $0.009\%$ and the
$\tau/e$ mass ratio to $0.34\%$; (xi)~a derivation of Newton's second law from the
...
regularization-independent observable; the absolute 2PN
coefficient remains an open finite-endpoint task), resolves the Schwarzschild
...
\textbf{Section~\ref{sec:mass_spectrum}} analyzes the mass spectrum
...
wave equation independent of $G$, establishes the
amplitude--frequency relation and demonstrates that mass ratios
are determined by the cavity geometry with gravitational
corrections of order $(m/\Mpl)^2 \sim 10^{-45}$, presents the
lattice formulation suitable for numerical solution, and connects
the lepton mass hierarchy to the Koide formula.
...
\textbf{Section~\ref{sec:birefringence}} computes one-loop
gravitational birefringence and the cosmological variation
...
(with four-velocity $u^\mu = (e^{-\varphi_0/2}, 0, 0, 0)$)
is
...
\item \textbf{MOND} (galactic): the galactic vortex
  ...
  Pulsons are driven outward, but those from the
  periphery cannot flow inward fast enough to
...
In this section we show that the mass hierarchy emerges naturally
from the eigenvalue spectrum of three-dimensional resonant
cavities and connects to the empirical Koide formula.
...
of~(\ref{eq:Veff_explicit}).  The mass of the $n$-th
resonance is
...
\textit{Mass ratios from the cavity spectrum.}---Within
the self-consistent confining potential, different angular
channels ($\ell$) and radial overtones ($n$) yield
distinct eigenfrequencies $\Omega_{n\ell}$.  The mass
ratios
\begin{equation}\label{eq:mass_ratios}
  \frac{m_{n_1\ell_1}}{m_{n_2\ell_2}}
  = \frac{\Omega_{n_1\ell_1}}{\Omega_{n_2\ell_2}}
\end{equation}
are determined by the geometry of the confining
potential---they depend on the mode numbers and the
cavity profile but are independent of $G$, $\hbar$, or
...
code \texttt{MASS/python/cavity\_vs\_mass\_audit.py}
and \texttt{cavity\_quantum\_oscillon.py}).  The identification
...
electron---utterly negligible.  The mass ratios are
therefore determined by the cavity geometry alone, with
gravitational corrections below $10^{-36}$ for every
known particle.
...
In the ISPG resonance framework, we propose that the
Koide relation is a \emph{geometric constraint} on the
eigenvalues of a three-dimensional resonator.  To see
...
\begin{enumerate}
  ...
  \item The specific form of the bi-conformal coupling,
    ...
    solution---the theory predicts a definite value of
    $x \equiv \alpha/\lambda_0$.  The Koide relation $Q = 2/3$
    is reproduced if and only if
    $x = 33 + 24\sqrt{2} \approx 66.9$.
\end{enumerate}

Computing $x$ from first principles requires solving the
full nonlinear eigenvalue system of
Sec.~\ref{sec:mass_determination}.  As shown there,
the mass ratios are determined by the geometry of the
self-consistent confining potential, independently of
$G$; gravitational self-energy corrections scale as
$(m/\Mpl)^2 \sim 10^{-45}$ and are negligible.
...
(numerical audit: \texttt{MASS/python/cavity\_vs\_mass\_audit.py}).
The Koide constraint also serves as a powerful independent
check: any candidate resonance spectrum that does not
satisfy $Q = 2/3$ can be excluded.
...
dominant effect, while the residual $0.11\%$ in $Q$ is
expected to close at second order.

The Hartree mechanism thus identifies the physical origin of
the $4/\pi$ correction: it is a first-order self-energy
shift of the electron eigenfrequency due to the ground-state
backreaction on the confining potential.

\textit{Quantum determination of $\eta$.}---In the QFT
framework, the coupling $\eta$ is not a free parameter
but is determined by the vacuum zero-point fluctuations of
the cavity modes.  For a mode with angular momentum
...
\begin{table*}[ht]
\centering
\caption{Falsifiable predictions of the quantum ISPG
  framework.  ``GR/SM'' denotes the prediction of general
  relativity and/or the Standard Model.}
...
schematic $\sim 10^{-7}$--$10^{-6}$ at $z \sim 1$--$3$ &
$0$ &
ESPRESSO, ELT-ANDES \\
\addlinespace
Gravitational decoherence rate &
...
$\Delta B = \pi/4$ (closed; absolute coefficient open per main Shapiro section) &
$0$ (by construction) &
SKA pulsar--BH \\
\addlinespace
...
\begin{enumerate}
  ...
  \item \textbf{Fine-structure constant variation}:
    ESPRESSO at the VLT is already collecting data
    at the required $10^{-7}$ sensitivity.
...
bounds: a maximum and a minimum.  If the substance of space is
water, it must be poured into a vessel; the vessel is the region
where the substance is absent.  One pole---\emph{substance}---represents
the extreme state in which pressure, the operational clock rate of
resonant structures, and spatial scale are asymptotically maximal.
The other pole---\emph{void}---represents
the null limit: zero pressure, no operational clock rate, zero size.
The void is not an independent entity; it is the logical
complement of substance---the necessary consequence of
something existing.  Physical reality is a bounded oscillation
between these two poles; a vibration that can never exceed
either limit.

This picture becomes intuitively transparent through a single
analogy: coloured soluble crystals in water.  The water is
space---the background medium.  The crystal is matter---a
localised, concentrated excitation within that medium.
As the crystal dissolves it releases colour into the
surrounding water, just as a particle (oscillon) emits
pressure-carrying tails into the surrounding space.  The
...
itself.  A particle (oscillon) behaves identically: it
vibrates in pressure and emits pressure tails into the
surrounding medium.  These tails reduce the background
pressure around the particle.  The reduction forces the
particle to shrink in effective size and mass
($\meff = m_0\,e^{\varphi/2}$).  A smaller, lighter particle
...
\begin{enumerate}
  ...
  \item \textbf{Mass hierarchy from 3D resonances}
    ...
    mass spectrum arises from the eigenvalues of a
    three-dimensional resonator.  The rank-1 structure
    of the bi-conformal coupling predicts $Q = 2/3$
    provided the self-consistent perturbation strength
    reaches $x \approx 66.9$---a condition whose
    verification requires numerical solution of the
    coupled oscillon equations
    (Sec.~\ref{sec:mass_determination}).  The mass
    ratios are fixed by the cavity geometry,
    independently of $G$, with gravitational self-energy
    corrections of order $(m/\Mpl)^2 \sim 10^{-45}$.
  ...
  \item \textbf{Gravitational birefringence}
    (Sec.~\ref{sec:birefringence}).  One-loop vacuum
    polarization in a gravitational gradient produces
    a polarization-dependent phase shift of order
    $\Delta c/c \sim 10^{-38}$ near neutron stars---far
    below current measurement sensitivity, but
    a distinctive theoretical prediction with no
    GR analog.
\end{enumerate}
...
independent of $G$; the mass ratios are determined by the
cavity geometry, with gravitational self-energy corrections
of order $(m/\Mpl)^2 \sim 10^{-45}$.  The lattice
...
Hartree correction recovers the Koide angle
$\theta_0 = 2/9$ to $0.07\%$;
...
\begin{equation}
  ...
  = e^{\varphi_0}|\nabla\varphi_0|^2 \geq 0.
\end{equation}
The static observer has $u^\mu = (e^{-\varphi_0/2}, 0, 0, 0)$, giving
(Eq.~\ref{eq:rho_phi}):
\begin{equation}
...
\textit{Key point: robustness under degeneracy-lifting.}---Now
...
perturbative regime of this bound; nevertheless, direct
evaluation of~\eqref{eq:Q_exact} with the measured masses
gives $|Q - 2/3| \approx 10^{-5}$, confirming the robustness
of the $Q = 2/3$ constraint far beyond the perturbative estimate.

This establishes that $Q = 2/3$ is a \emph{robust}
consequence of an approximately rank-1 perturbation of
a degenerate system, valid for \emph{any} perturbation
strength $\alpha$, with corrections controlled by
the secondary splitting $\epsilon$, not by $\alpha$ itself.
\hfill $\square$
