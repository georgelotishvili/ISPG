...
Sec.~\ref{sec:formation_time} (using the full quantum-feedback
$\mathcal{C}(z)$ profile, not the Appendix~18 power-law benchmark),
one finds
...
The effective process-time available for vortex formation can exceed
the standard coordinate age $t_0\simeq13.8\;\mathrm{Gyr}$ measured by
present-day clocks.
...
($z_f=10$, $\alpha=1$, $K\approx 8$), using the full
quantum-feedback $\mathcal{C}(z)$ profile rather than the
Appendix~18 power-law benchmark, one obtains
...
Within ISPG, this rotation-curve support is the direct signature of the
second channel; ruling out alternative MOND/dark-sector explanations
requires the full ISPG signature programme (Sec.~\ref{sec:open_programme}).
In this interpretation, space behaves as a gravitating substance whose
pressure and transport state is encoded in the effective gravitational
field: rotation rearranges the medium, produces a central rarefaction,
and the resulting transported channel contributes a genuine second
metric/geodesic component to the gravitational response.  The mechanism is
...
Source-free non-axisymmetric transients decay under Hubble damping at rate $\sim3H/2$, so over several Hubble times such free modes are exponentially suppressed relative to the $m=0$ envelope. Continuously driven spiral sidebands ($m=\pm1$) are not source-free; they feed the averaged $m=0$ envelope through the sideband-beat mechanism developed in the mode-mixed WKB reduction below. The focus on $\varphi_0$ here is therefore on the averaged $m=0$ mode supplied by sideband beating, not on a trivial absence of $m\neq0$ structure.
...
\paragraph{Bessel cell length and crossing time.}
The Bessel eigenvalue $k_r=2.4048/r_0$ sets a cell length
$\ell_B=1/k_r=r_0/2.4048$ and a Bessel crossing/spatial-adjustment time
$t_B=\ell_B/c=1/(c\,k_r)=r_0/(2.4048\,c)$, matching the formula used in
\texttt{MOND/Python/multiscale.py}.

\paragraph{Transport-balance relaxation time.}
The transport-balance relaxation/decorrelation time
\begin{equation}
\tau_{\rm rel} = \frac{c}{g},
\label{eq:tau_rel_derived}
\end{equation}
is uniquely selected within the rotating-medium transport ansatz by
dimensional analysis and universality: it is the only galaxy-independent,
dimensionally consistent choice within the rotating-medium transport
closure. A direct PDE-level derivation of the transport ansatz remains
the next theorem-level strengthening.

\paragraph{Status note on the two times.}
$t_B$ is a Bessel-eigenvalue spatial-adjustment time and is not the same
object as the transport-balance relaxation time $\tau_{\rm rel}$ used in
the master equation below.

This transport-balance time is \textbf{unique by dimensional analysis}
...
coherent packets lose phase memory at rate $\Gamma_{\rm mem}$.  Within this
minimal bounded two-state Markov closure, the stationary coherent fraction is
\begin{equation}
P_C
...
earlier in Sec.~\ref{sec:a0}.  This identification is derived within the
coherence-boundary / bifurcation master-equation closure of
Secs.~\ref{sec:a0} and~\ref{sec:dimless}; a full microscopic/action-to-PDE
derivation remains the next theorem-level strengthening.  The phenomenological
success of the resulting $\mu(x)=x/(1+x)$ law below is an empirical check on
the unity-coefficient choice.
...
\paragraph{Consistency domain.}\label{sec:consistency_domain}
...
\paragraph{Mathematical target vs implemented surrogate.}
The mathematical target of the source-side kernel is the unique AQUAL
phantom-density / Green-function object for the well-posed elliptic
BVP~\eqref{eq:aqual_from_ispg}.  The implementation in
\texttt{MOND/Python/pde\_2d\_swirl\_verify.py} and the blind
self-calibrated 2D kernel of
\texttt{phase\_al\_blind\_selfcalibrated\_kernel\_2d.py} uses a
boundary-normalized, enclosed-mass numerical Green-kernel surrogate
with explicit regularity-fixed core, asymptotic-annulus matching, and
Green prefactor; the surrogate approximates the unique mathematical
object with the quantified residuals reported in
Sec.~\ref{sec:tier_residuals}.
...
$A_{\rm vort}(r_M,0)\sim9.99\times10^{-1}$).  Thus the
algebraic/transport-side closure gap on the mature branch is closed at high
precision (RMS $<10^{-3}$); source-side, operator-side, and coupled
hyperbolic $2+1$D channels are independent positive/convergent realizations
with quantified residuals at the few-to-10\% level
(Sec.~\ref{sec:tier_residuals}).
...
\item \textbf{Operating rate:} once activated, the transported
  ...
  \emph{unique stationary} galaxy-independent transport rate within the
  coherence-boundary master-equation closure allowed by hyperbolic propagation
  at speed $c$ together with the outer coherence boundary
  $\lambda_H=2\pi c/H$
...
disk, the leading-order dimensionally normalized rotating-medium master
equation used for the WKB/transport analysis below can be written as
\begin{equation}
\boxed{
\ddot{\varphi}
+ 3H\,\dot{\varphi}
...
\eqref{eq:dimless_coords}--\eqref{eq:dimless_field}.  The
$3H\,\Delta_{\rm FD}$ coefficient is this master-equation normalization;
a direct derivation from the metric cross-term
$2g^{t\phi}\partial_t\partial_\phi$ belongs to the
symbolic/rotating-PDE verification programme.  Equation
...
(Open programme, item~2).  The PDE-to-$\mu$ chain has no remaining algebraic
or transport-side closure gap on the mature branch; the Green-kernel/profile
realization residual at the few-to-10\% level remains the open quantitative
refinement.
...
The baryonic Tully--Fisher relation (BTFR) follows directly from the interpolating function $\mu(x)=x/(1+x)$ in the isolated, asymptotic, deep-MOND, circular-orbit, finite-baryonic-mass, negligible-EFE, mature vortex-equilibrium ($C_{\rm eff}\simeq 1$) regime. The derivation chain is:
...
\end{equation}
This boxed relation uses the instantaneous/local-equilibrium
$a_0=cH_0/(2\pi)$.  When the vortex-lag/formation-age
correction of Sec.~\ref{sec:vortex_lag} is active, the same
asymptotic relation reads $v_{\rm flat}^4=a_{0,{\rm eff}}\,GM$
for the relevant galaxy history; this preserves the
$v^4\propto M$ exponent and absorbs age/history dependence into
the per-galaxy normalization.  For this instantaneous baseline,
the normalization $a_0 = cH_0/(2\pi)$ is selected by the
cosmological coherence boundary (Sec.~\ref{sec:a0}), so the BTFR
emerges with \emph{no galaxy-by-galaxy fit parameters}.  The
asymptotic velocity depends only on baryonic mass $M$ and the
universal constant $a_0$.
The clean $v^4=GMa_0$ normalization above uses the mature
instantaneous vortex-equilibrium branch where
$C_{\rm eff}\simeq1$ (Sec.~\ref{sec:consistency_domain}).  The
accumulation contrast model
(\texttt{MOND/Python/ceff\_asymptotics.py}) gives outer-radii
$C_{\rm eff}<1$ and shifts the per-galaxy normalization by tens of
percent while preserving the $v^4\propto M$ exponent; that branch
is not the BTFR baseline of this section.
...
\label{eq:btfr_z}
\end{equation}
In the ISPG comparison background---the baryons-only cosmology
natural to this framework ($\Omega_b \approx 0.05$,
$\Omega_\Lambda \approx 0.95$)---$H(1)/H_0 \approx 1.16$,
predicting $(1.16)^{1/4} - 1 \approx 4\%$ higher asymptotic
velocities at $z = 1$ at fixed baryonic mass---a falsifiable
signature distinguishing this theory from standard MOND with
constant $a_0$.  Here again $H(z)$ should be read as the
observational proxy for the underlying pressure-history variable.
(For comparison, a $\Lambda$CDM cosmology with
$\Omega_m \approx 0.3$ would give $H(1)/H_0 \approx 1.78$ and a
$\sim$15\% velocity increase.)  This baryons-only
redshift-evolution table is a comparison background distinct from
the standard-$\Lambda$CDM observational background used for the
H.3 $H_{\rm eff}$ calibration in Sec.~\ref{sec:vortex_lag};
reconciling the two backgrounds is deferred to a separate
...
is an order-of-magnitude survey-design forecast in the regime where
the three scenarios start to separate; an explicit scatter,
selection, inclination, mass-error, and redshift-bin analysis is
required to convert this into a formal significance estimate.
...
\begin{equation}
\mu_{\rm obs}(x) = \frac{x}{1+x} + \mathcal{O}(10^{-3}) \quad \text{(target RMS error on the algebraic/transport-side benchmark)},
...
background.  The total Newtonian/source-side acceleration at
position $\mathbf{r}$ inside the satellite is
\begin{equation}
  \mathbf{g}_N(\mathbf{r})
  \;=\;
  \mathbf{g}_{N,{\rm int}}(\mathbf{r})
  \;+\;
  \mathbf{g}_{N,{\rm ext}}\,,
\label{eq:g_total_efe}
\end{equation}
where $\mathbf{g}_{N,{\rm int}}$ is generated by the satellite's
own baryons and $\mathbf{g}_{N,{\rm ext}}$ is the host contribution
on the Newtonian/source side.  The physical MOND-side total field is
$\mathbf{g}=\mathbf{g}_{\rm int}+\mathbf{g}_{\rm ext}$, where
$\mathbf{g}_{\rm ext}$ is the physical external background field.
As a local algebraic surrogate for the full nonlinear EFE
boundary-value problem, the MOND interpolating function acts on the
\emph{magnitude} of this physical total field:
\begin{equation}
  \mu\!\left(\frac{|\mathbf{g}|}{a_0}\right)\,\mathbf{g}
  \;=\;
  \mathbf{g}_N\,.
\label{eq:efe_mu}
\end{equation}
A precision EFE calculation requires the corresponding
nonlinear/vector field equation with the external boundary condition;
Eq.~\eqref{eq:efe_mu} is the limiting-regime algebra used below.
...
dynamics have a local first-order directional correction
proportional to $(g_{\rm ext}/g_{\rm int})\cos\alpha$.  After
angular/orbital averaging the isotropic correction begins at
order $(g_{\rm ext}/g_{\rm int})^2$, which is negligible for massive
spirals in the field.

\paragraph{(iii) Strong external field ($g_{\rm ext} \gg g_{\rm int}$,
$g_{\rm ext} \lesssim a_0$).}
Expanding the local algebraic surrogate~\eqref{eq:efe_mu} to linear
order in $g_{\rm int}/g_{\rm ext}$ gives a projected scalar estimate
for the \emph{internal} response,
\begin{equation}
  \mu_{\rm eff}
  \;\approx\;
...
\end{equation}
This scalar $\mu_{\rm eff}$ is a linear local approximation to the
anisotropic EFE response, not the full vector/tensor solver.  In this
limit the internal dynamics are \emph{quasi-Newtonian}: the isolated
deep-MOND enhancement $g \sim \sqrt{a_0 g_N}$ is suppressed and
replaced, in the projected scalar estimate, by
$g \sim g_N/\mu_{\rm eff}$ with $\mu_{\rm eff}$ bounded below by
$\mu(x_e)>0$.
...
\begin{enumerate}
  \item \textbf{Dwarf satellites.}  Satellites of the Milky Way
    ...
    show velocity dispersions consistent with \emph{quasi-Newtonian,
    EFE-suppressed} dynamics, with directional anisotropy in the host
    direction; full Newtonianization requires $g_{\rm ext}\gg a_0$.
    Isolated dwarfs at comparable $M_{\rm bary}$ should show the full
  ...
  \item \textbf{Wide binaries.}  Stellar pairs with separations
    ...
    reduces the isolated MOND boost: a residual scalar velocity
    enhancement remains, while the cleaner ISPG/MOND discriminator is
    the directional anisotropy of the internal response.  Current
    \emph{Gaia} DR3 wide-binary analyses are contested and
    constraining for standard MOND-like boosts
    \cite{Banik2024GaiaWB,Pittordis2025GaiaWB}; see also the critical
    review in Ref.~\cite{Hernandez2024GaiaWB}.
    ...
    the relative-orbit Newtonian acceleration
    $g_{\rm int}=G(m_1+m_2)/s^2$.
    At $s=10\,000\;\mathrm{AU}$:
    $g_{\rm int}/a_0\approx0.99$ for
    $a_0=1.2\times10^{-10}\,\mathrm{m\,s^{-2}}$,
    giving $\mu_{\rm eff}\approx0.643+0.13\cos\alpha$.  The
    $\alpha$-averaged scalar estimate gives
    $\Delta v/v_{\rm Kepler}\approx25\%$; the small-enhancement
    expansion $\tfrac12(1/\mu_{\rm eff}-1)$ gives $\approx28\%$
    and is only a rough expansion at this amplitude.  This excess is
    the EFE-reduced MOND signal, not the full isolated-MOND boost.
    The linear expansion~\eqref{eq:mu_eff_efe} is valid only
    for $g_{\rm int}\ll g_{\rm ext}$; already at
    $s=5\,000\;\mathrm{AU}$ one has $g_{\rm int}/a_0\approx4>x_e$,
...
\subsection{Numerical PDE verification (status)}\label{sec:tier_residuals}
...
\item Target accuracy on the algebraic/transport-side benchmark: RMS error
  $<10^{-3}$ across $x\in[10^{-2},10^2]$
\end{itemize}

The fiducial disk parameters are labelled per numerical surface:
\texttt{MOND/Python/mond\_numerical\_verification.py} and its PDF
figures use a MW-like disk with $R_d=3.5\,{\rm kpc}$, while the
source-kernel fiducial derivation later in this section uses
$R_d=10\,{\rm kpc}$.  Both are representative mature-spiral
fiducials; the larger value is convenient for the kernel-analytic
limit.
...
\footnotesize Coupled evolving-source 2+1D &
  ...
  \footnotesize$\sim\!14\%$ \\
\end{tabular}\\[0.2em]
All six channels converge to the same static branch (mutual field RMS
...
BTFR exponent $v^4\propto M$ confirmed.  The EFE/wide-binary
sector remains an open discriminator with directional anisotropy as
the cleaner test; current \emph{Gaia} DR3 analyses are contested and
constraining for standard MOND-like boosts.\\[0.5em]
%
...
\subsection{Nature of the residual 2D mismatch (curl-field origin identified)}\label{sec:2d_mismatch}

The source-side and operator-side 2D benchmarks consistently show
$\sim6$--$13\%$ field RMS mismatch against the 1D algebraic target
...
\item \textbf{All 2D channels converge to a common branch.}
  ...
  branch with mutual field RMS discrepancies of $\sim4$--$7\%$.  This
...
With the curl-field origin established, the residual is identified
as a known geometric feature of MOND in disk geometry, not a
derivation gap.  The $\mu$-profile (the direct observable) is
verified to $\sim2$--$5\%$ across multiple independent channels and
galaxy types; the full-field mismatch reflects the AQUAL curl in
disk geometry.  A closed-form Green-kernel realization at precision
level remains the open quantitative refinement.
...
\item[$\to$] Load/free odds ratio $R=\sqrt{y}$ and
  ...
  $+$ local MaxEnt)
  \hfill\textbf{MaxEnt-selected within binary free/load partition}${}^{\dagger}$
...
\item[$\to$] Impedance-matching invariant $\hat I=2K_{\rm rot}^2A_{\rm vort}$
  ...
  \hfill\textbf{Unique by impedance matching within power-law closure family}${}^{\ddagger}$
\item[$\to$] Hubble-boundary vortex equilibrium rate $\Omega_{\rm tr}=a_0/c$
  ...
  \hfill\textbf{Derived within coherence-boundary closure}${}^{\P}$
...
free inputs. The derivation is within the rotating-medium
master-equation/coherence-boundary closure; a full action-to-PDE
derivation remains the next theorem-level strengthening.\\
...
spiral-galaxy window.  Systems without a mature macroscopic vortex
(Solar System, laboratory) sit on the small effective-activation branch;
there $A_{\rm vort}$ is not the local orbital-frequency fraction
(Sec.~\ref{sec:consistency_domain}).
...
\begin{itemize}
  ...
  \item \textbf{Galaxy clusters.}  Superfluid dark matter naturally
    ...
    time ($\sim\!0.4\,t_{\rm dyn}$) is a conditional cluster-flow
    benchmark and ICM cold-front scattering is the proposed
    retention mechanism; a dedicated 3D cluster solve is in
    preparation.
...
\section{Equatorial concentration: vertical confinement from rotational anisotropy}\label{sec:disk}

The preceding sections assumed a thin-disk geometry as an \emph{input}.
Here we show that the same rotational-sector closure that produces the
MOND interpolating function also supplies an additional
$z$-directed restoring gradient toward the equatorial plane.  This is a
vertical-confinement mechanism for already rotating baryons, not a full
disk-formation calculation including angular-momentum acquisition, gas
cooling, feedback, and stability modelling.
...
\paragraph{Status of the angular extension.}
The $\sin^2\theta$ factor in \eqref{eq:phih_theta} is inherited from
the frame-dragging geometry, while the amplitude
$\varphi_h^{(\rm eq)}$ is the mature coherent-vortex branch derived in
Sec.~\ref{sec:mu}.  Thus the estimate does not use the
$\epsilon$-suppressed linear frame-dragging trigger as the MOND
amplitude.  What is not yet done is a full 3D scalar/vortex PDE solve
that derives the complete angular profile and disk-settling history
from first principles.

\noindent The total scalar field perturbation is:
\begin{equation}
...
In standard astrophysics, disk formation depends on angular-momentum
acquisition, gas cooling, feedback, and stability.  The scalar sector
does not replace that formation problem.  In the present theory, the
rotational anisotropy of the transported scalar sector provides an
\emph{additional} vertical confinement mechanism that is absent in GR
without dark matter.  The two descriptions diverge most strongly for
low-surface-brightness galaxies deep in the MOND regime, where the
rotation-induced contribution to vertical confinement is largest
relative to the Newtonian component.
...
(the Faber--Jackson analogue of the BTFR).  We now separate the
closed deep-MOND virial scaling from the model-dependent
medium-response route that would realize the same MOND branch in
...
a qualitative topology map for elliptical galaxies that does not depend on
...
\paragraph{Why $\mu(x)$ is expected to be universal.}
...
coefficient is taken to match the spiral branch in the working
medium-response extension because the Hubble-damping scale
$a_0 = cH/(2\pi)$ is a \emph{cosmological} parameter, independent of
the galaxy's internal kinematics.  The exact interpolating function
away from the deep-MOND limit remains a pressure-supported-system PDE
check rather than a completed derivation in this subsection.
...
\end{enumerate}
In this working pressure-supported extension, isolated dSphs obey
\eqref{eq:faber_jackson}, while those embedded in a strong external
field ($g_{\rm ext} \gtrsim a_0$) show suppressed MOND effects.
This differential signature is directly testable, but its detailed
normalization inherits the convective-cell/PDE status described above.
...
In the present framework no new fundamental particle species is
introduced.  The benchmark instead uses the effective $\chi$ sector
already present in the macroscopic hysteresis closure.  The Bullet
morphology emerges in the frozen-hysteresis closure obtained by combining
this effective sector
...
\textit{Outputs for fixed calibrated inputs.}---Given these inputs,
the solve yields the following results that were not separately imposed
by the aperture-mass calibration:
\begin{itemize}
  ...
  \item \textbf{Contrast ratio (benchmark output).}
    ...
    Neither the contrast nor the peak-to-peak asymmetry is separately
    fitted in this run; they follow after the observed gas geometry,
    aperture masses, $f_{\rm hyst}$, and $L_\chi$ are fixed.
  \item \textbf{Mass budget (non-trivial consistency test).}
    ...
    the gas widths, $L_\chi$, and $f_{\rm hyst}$, which are fixed
    before checking the decomposition.  The
    solve yields $f_\chi\simeq0.79$ for the main cluster
    ($\langle f_\chi\rangle=0.763$ across the two outer
    apertures), compatible with the
...
$f_{\rm hyst}=1.3$.  Benchmark output: the total convergence
...
mechanism can reproduce the observed Bullet Cluster morphology in a
calibrated snapshot once the effective $\chi$ sector, the freeze
hierarchy, the observed baryonic geometry, and the order-unity
$(f_{\rm hyst},L_\chi)$ benchmark are imposed.  No new fundamental
field, particle, or external dark-matter species is required, but the
macroscopic $\chi$ field is an effective collective degree of freedom
and the current result is a compatibility benchmark rather than a full
time-dependent merger prediction.  A hydrodynamic $N$-body+$\chi$
simulation remains the quantitative closure test for the Bullet system.
...
In the present framework, the missing binding can have a natural
...
\;\approx\; 0.42\,.}
\end{equation}
The resonant-tail energy must remain in the cluster for
$\sim\!40\%$ of one dynamical time.  This is a plausible
flow-scale residence time in a bound intracluster medium, but it is a
condition of the benchmark, not yet a theorem: if the long-wavelength
tail background escapes ballistically on the light-crossing time, the
cluster residual remains open, exactly as the numerical script reports.
...
The required $\ell_{\rm mfp}\approx16\;\mathrm{kpc}$ is below the
simple geometric interface spacing but comparable to the observed
cold-front thickness scale, making resonant scattering a plausible
target.  A wave-scattering calculation on observed ICM profiles is
still needed before this can be promoted from a consistency estimate to
a derived mean free path.
...
($\sim\!50\%$ of the discrepancy) follows once the AQUAL BVP is assumed from
...
The feedback-ODE benchmark and the dynamical-time argument show that
the binding amplitude can reach the required range if the
long-wavelength tail background is retained on a cluster flow time.
...
weak-lensing data.
These are concrete computations within the existing framework and are
required for quantitative cluster closure.
...
\end{equation}
where $\mathcal N_{\rm act,bg}$ is the background form of the
measurable-existence identification available to matter standards, not an
independent number-density count.  This reading does not replace the transport and
...
7 & Load/free law $Z_{\rm rot}=1/(1+\sqrt{y})$
    ...
    and local MaxEnt
  & MaxEnt-selected within binary free/load partition${}^{\dagger}$ \\
8 & Impedance matching ($n=2$ uniquely cancels $K^{-2}$ in $Z'$)
    $\to \hat I=2K^2A_{\rm vort}$
    $\to \Upsilon_{\rm rot}=A_{\rm vort}a_0/g$,
    $C_{\rm eff}=A_{\rm vort}$
  & unique by impedance matching within power-law closure family${}^{\dagger\dagger}$ \\
...
10 & Hubble-boundary vortex equilibrium rate
  ...
  & derived (coherence-boundary closure)${}^{\P}$ \\
...
rather than being a free closure input.  The rate derivation is within the
rotating-medium master-equation/coherence-boundary closure; a full
microscopic/action-to-PDE derivation remains the next theorem-level
strengthening.  The saturated value $C_{\rm eff}\simeq1$ applies on the
mature macroscopic occupied branch ($Q_{\rm occ}\gg1$,
$\delta_{\rm sp}\ll1$, $N_{\rm cell}\gg1$), not as a universal local
orbital-frequency rule for non-galactic systems.
...
\item \textbf{Closed-form AQUAL Green function in cylindrical geometry.}\\
  ...
  with the algebraic/transport-side closure gap closed; the
  Green-kernel/profile residual remains a quantitative refinement.\\
...
\item \textbf{Cluster binding: 3D steady-state solve.}\\
  ...
  (Sec.~\ref{sec:cluster_binding}) shows that the $50\%$
  binding threshold can be reached if the tail background is retained for
  $\tau_{\rm ret}\approx0.63\;\mathrm{Gyr}\approx0.42\,t_{\rm dyn}$,
  rather than escaping ballistically.  In the diffusion picture, the
  corresponding mean free path $\ell_{\rm mfp}\approx16\;\mathrm{kpc}$
  is a plausible resonant-scattering target tied to observed ICM
  cold-front structure, but it has not yet been derived from a
  wave-scattering calculation.\\
  ...
  weak-lensing data.\\
  \textit{Output:} derived $\rho_{\rm extra}/\rho_{\rm cl}$
  profile replacing the current calibrated benchmark, plus a
  prediction for binding--substructure correlation.
...
\item \textbf{Elliptical-galaxy convective-cell: explicit PDE check.}\\
  ...
  Sec.~\ref{sec:convective_cell} motivates the working hypothesis that the
  Bernoulli pressure-deficit mechanism, the coherence boundary
  $\lambda_H$, the forgetting rate $\Gamma_{\rm mem}$, and the
  Markov activation chain are topology-independent, so the spiral
  $\mu(x)=x/(1+x)$ branch may carry over to ellipticals with
  $\omega\to\sigma/r$.  The deep-MOND Faber--Jackson virial
  coefficient is derived exactly once the deep-MOND force law is
  assumed (Sec.~\ref{sec:ellipticals}); the convective-cell PDE
  ...
  numerical $\mu$ profile agrees with $x/(1+x)$.  This is the
  required quantitative closure test for the pressure-supported
  extension, not for the already-closed spiral algebra.\\
...
under $O(1)$ rescaling of $a_0$; the deep-MOND virial
scaling that yields the baryonic Faber--Jackson relation
$\sigma^4 = K_\sigma\,GMa_0$ once the MOND force law is
assumed (Sec.~\ref{sec:ellipticals}); the turbulent/convective
medium-response realization for pressure-supported systems remains
a model-dependent extension; and the EFE with its
...
asymptotics, achieving $\sim6$--$13\%$ field RMS on the compact
matched box and $\sim10\%$ field RMS on the widened disjoint-annulus
benchmark; the operator-side AQUAL solve at
$\sim11\%$ field RMS on the compact box and $\sim9\%$ field RMS on the widened
benchmark, with $\mu$ supplied as input; universality across
galaxy parameters; spectral convergence; Poisson-source
grid-verified positivity for the occupied branch; and fixed-source/coupled
...
a vertical-confinement mechanism from frame-dragging equatorial
...
\bibitem{Banik2024GaiaWB}
I.~Banik, C.~Pittordis, W.~Sutherland, B.~Famaey, R.~Ibata,
S.~Mieske, and H.~Zhao,
...
\bibitem{Hernandez2024GaiaWB}
X.~Hernandez,
``A critical review of recent Gaia wide binary gravity tests,''
Mon.\ Not.\ R.\ Astron.\ Soc.\ \textbf{533}, 729 (2024),
doi:10.1093/mnras/stae1823.

\bibitem{Pittordis2025GaiaWB}
C.~Pittordis, W.~Sutherland, and P.~Shepherd,
``Wide Binaries from Gaia DR3: testing GR vs MOND with realistic triple modelling,''
Open J.\ Astrophys.\ \textbf{8} (2025),
