...
\item the linear cosmological sector remains GR-identical as established in the CMB-consistency analysis of the main theory (no modification of the Einstein--Boltzmann hierarchy);
...
\begin{itemize}
\item \textbf{Qualitative (cosmological origin):} observed cosmic flows in the space substance $\rightarrow$ vortex formation (Kelvin--Helmholtz) $\rightarrow$ central rarefaction $\rightarrow$ extra metric/geodesic acceleration channel $\rightarrow$ MOND phenomenology.  Different vortex topologies (laminar planar, convective poloidal, chaotic) produce different galaxy types (spiral, elliptical, irregular) with the same MOND scale.
\item \textbf{Quantitative (local closure):} global pressure decrease / coherence damping $\rightarrow$ coherence length $\rightarrow$ critical acceleration $a_0=cH/(2\pi)$ $\rightarrow$ vortex-lag correction $H\!\to\!H_{\rm eff}$ (best-fit recovery of the local observed value) $\rightarrow$ instantaneous vortex equilibrium $\rightarrow$ interpolating function $\mu(x)=x/(1+x)$ $\rightarrow$ BTFR and environment effects.
\end{itemize}
...
medium), not an empty container.  The observed large-scale cosmic flows
are therefore bulk motions \emph{of} that substance.  We now show that
the existence of galactic-scale vortices is a \emph{necessary consequence}
of this ontology together with well-established observational facts.
...
\subsection{From flows to vortices: a fluid-dynamical inevitability}\label{sec:flows_to_vortices}

In any real fluid with inhomogeneous flows, vortex formation is
inevitable:
\begin{itemize}
  ...
  \item No external ``spinning agent'' is required: differential
    ...
    gradients drive flows, and flows at different angles inevitably
    create vortices---precisely as atmospheric temperature differences
...
the three-dimensional topology of cosmic flows and weak
compressibility modify the coefficient by $O(1)$ geometric factors
but not the qualitative result that $\gamma_{\rm KH}^{-1}$ remains
well below the formation window estimated below.
...
the inertial range.  In other words, once cosmological flows exist, the
generation of galactic-scale vortices is not merely qualitative rhetoric
but a quantitatively expected outcome.
...
\caption{Quantitative vortex-origin diagnostics from
\texttt{phase\_ao\_kh\_vortex\_origin.py}. Left: the Kelvin--Helmholtz
e-fold time stays far below the proper-time-weighted formation window
across the $10$--$1000\;\mathrm{kpc}$ range, so cosmological shear has
ample time to roll up into vortices. Right: with the conservative
viscosity ceiling implied by vortex persistence, the dissipation scale
lies far below galactic scales, leaving the full $10$--$100\;\mathrm{kpc}$
band safely inside the inertial range.}
...
\end{equation}
Each arrow is either an observed fact or a textbook fluid-dynamical law:
\begin{center}
\begin{tabular}{@{}ll@{}}
\toprule
...
Inhomogeneous cooling $\to$ gradients & thermodynamics \\
Gradients $\to$ flows & fluid mechanics \\
Cosmic flows exist & observed (Laniakea, cosmic web) \\
Flows $\to$ vortices & Kelvin--Helmholtz, proved \\
Vortices trap matter & fluid mechanics \\
\bottomrule
\end{tabular}
\end{center}
...
different vortex topologies, which map directly onto the Hubble
sequence:
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
...
\emph{geometry} of the extra support but not its \emph{existence}.
...
\label{eq:jeans}
\end{equation}
Modes with $\lambda \gg \lambda_J$ are overdamped (coherent response), while modes with $\lambda \ll \lambda_J$ oscillate (incoherent response). This defines the coherence length of the scalar response.
...
The damped oscillator~\eqref{eq:damped} has two qualitative regimes in the sub-Hubble adiabatic range ($kc/a \gg H$), separated by the critical condition $\omega_k = \gamma_H$:
\begin{itemize}
\item \textbf{Underdamped} ($\omega_k > \gamma_H$): the mode oscillates with amplitude decay rate $\gamma_H$.  The field can sustain a coherent gradient at the corresponding wavelength and delivers the full Newtonian response.
\item \textbf{Overdamped} ($\omega_k < \gamma_H$, still sub-Hubble): the mode relaxes without oscillating.  Its approach to equilibrium is controlled by the slow eigenrate
\begin{equation}
...
\label{eq:alpha_slow}
\end{equation}
giving a response timescale $\tau_{\rm resp}=1/\alpha_-\approx 3H/\omega_k^2$ that exceeds the Hubble time for $\omega_k^2 < 3H^2$.  Such modes cannot reach equilibrium within a Hubble epoch; their gravitational gradient is suppressed.
...
The acceleration corresponding to a spatial gradient of length $\ell$ is $a(\ell)=c^2/\ell$.  The Hubble friction suppresses gradients with $\ell$ exceeding the coherence length, which is of order $c/H$ regardless of the precise criterion used:
\begin{center}
\begin{tabular}{@{}lcc@{}}
\toprule
...
\end{tabular}
\end{center}
All three give $a_0 = \mathcal{C}\,cH$ with $\mathcal{C}$ an $O(1)$ coefficient.  The scaling $a_0\propto cH$ is therefore \emph{robust}: it follows from any reasonable demarcation of the overdamped regime.
...
\label{eq:a0}
\end{equation}
Given the Hubble-wavelength identification, the factor $2\pi$ follows from the standard Fourier relation $k=2\pi/\lambda$ between wavenumber and wavelength; the wavelength-vs-radius choice itself is a physical modeling decision, justified by the coherence structure of the scalar medium and confirmed by the vortex-lag direction constraint below.

\paragraph{Direction constraint from the vortex-lag mechanism.}
Among the three criteria of the preceding subsection, only the Hubble-wavelength criterion yields an instantaneous value $a_0^{\rm inst}=cH_0/(2\pi)\approx 1.04\times10^{-10}\,\mathrm{m\,s^{-2}}$ \emph{below} the observed $a_0^{(\rm obs)}\approx 1.2\times10^{-10}\,\mathrm{m\,s^{-2}}$.  The vortex-lag mechanism of Sec.~\ref{sec:vortex_lag} enhances $a_0^{\rm inst}$ via past-weighted averaging $H_{\rm eff}>H_0$, and can therefore close the gap only for criteria that \emph{underpredict} the data.  This provides a second, mechanism-level selection principle complementary to the Fourier identification: the two overpredicting criteria ($3cH/(4\pi)$ and $\sqrt{3}\,cH/(2\pi)$) would require a lag correction of the wrong sign and are excluded once the vortex-lag closure is imposed.
...
\subsection{Resolving the 15\% offset: vortex-lag effect}\label{sec:vortex_lag}

The instantaneous formula $a_0 = cH_0/(2\pi) \approx 1.04\times10^{-10}\;\mathrm{m\,s^{-2}}$ under-predicts the observed value $a_0^{(\rm obs)}\approx 1.2\times10^{-10}\;\mathrm{m\,s^{-2}}$ by $\approx 15\%$.  This is not within acceptable tolerance for a parameter-free derivation: the discrepancy must be physically resolved.
...
\end{equation}
where the second equality follows from the change of variable $dt = dz/[(1+z)H]$ and the identity $\int H\,dt = \int dz/(1+z)$.
...
For a standard $\Lambda$CDM background ($\Omega_m=0.315$, $\Omega_\Lambda=0.685$, $H_0=67.4$\,km\,s$^{-1}$\,Mpc$^{-1}$, giving $t_0\approx 13.8$\,Gyr):
\begin{center}
\begin{tabular}{@{}ccccl@{}}
\toprule
...
\end{tabular}
\end{center}
\noindent The observed $a_0^{(\rm obs)}\approx 1.2$ corresponds to $z_{\rm form}\approx 0.58$, i.e.\ the vortex structure dates from $\sim$5.8~Gyr ago.  This is consistent with the epoch at which the last major merger typically stabilises spiral-disk morphology in the local galaxy population~\cite{Hopkins2010,Stewart2008}.
...
This resolution generates a unique, testable prediction absent from any other MOND formulation:
...
\item Current SPARC galaxies are predominantly local spirals with $z_{\rm form}\sim 0.5$--$0.7$, qualitatively consistent with both the observed median $a_0\approx 1.2$ and the narrow BTFR scatter (a quantitative verification of the induced dispersion $\sigma(\log v_{\rm flat})=\tfrac14\sigma(\log a_0)=\tfrac14\sigma(\log H_{\rm eff})$ against the SPARC scatter is deferred to a dedicated analysis).
\end{itemize}
...
Several independent data sets are already qualitatively consistent with the vortex-lag prediction:
\begin{enumerate}
\item \textbf{MIGHTEE-HI (2025).}
  Villanueva-Domingo et al.~\cite{MIGHTEE2025} report ``the first
  tentative evidence for redshift evolution in the acceleration
  scale'' from 19 HI-selected galaxies at $z\leq0.08$.  A
  redshift-dependent $a_0$ is the direct signature of the
  vortex-lag mechanism.
\item \textbf{Galaxy-to-galaxy $a_0$ scatter (Rodrigues et al.\ 2018).}
  ...
  scatter is \emph{physical}: it encodes the distribution of
  disk-settling epochs $z_{\rm form}$ across the sample.
\item \textbf{Best-fit $a_0$ value.}
  ...
  0.58$ falls within the independently determined
  $z\sim 0.5$--$0.7$ window for last-major-merger
  stabilisation of local-spiral disk
...
\end{equation}
The normalization $a_0 = cH_0/(2\pi)$ is fixed by the cosmological sector (Sec.~\ref{sec:a0}), so the BTFR emerges with \emph{no galaxy-by-galaxy fit parameters}. The asymptotic velocity depends only on baryonic mass $M$ and the universal constant $a_0$.
...
If the frame-dragging coupling is switched off ($\bm{\omega}_{\rm FD} = 0$), the scalar field reduces to the pure overdamped oscillator of Eq.~\eqref{eq:damped}, giving
\begin{equation}
...
\label{eq:btfr}
\end{equation}
the baryonic Tully--Fisher relation, with normalization fixed by \eqref{eq:a0} and its redshift drift by \eqref{eq:a0z}. A useful scaling is
\begin{equation}
...
\textbf{Linear cosmology (does not change).} By construction, and as shown in the CMB analysis of the main theory~\cite{Lotishvili2026}, the linear Einstein--Boltzmann system on FLRW is GR-identical in the main theory backbone. In particular, we do \emph{not} introduce a linear-scale $G_{\rm eff}(z)\neq G$ here.

\textbf{Nonlinear bound structures (can change).} MOND phenomenology enters through the quasi-static response around bound structures and therefore affects nonlinear structure (galaxy rotation curves, satellite dynamics, lensing--dynamics relations) while remaining compatible with linear CMB constraints.
...
In ISPG the coincidence is \emph{derived}: the scalar field equation
on the FLRW background has a Hubble-damped coherence length
$\lambda_H=2\pi c/H$, and the critical acceleration is
$a_0=c^2/\lambda_H=cH/(2\pi)$ (Sec.~\ref{sec:a0}).  The $2\pi$
factor is fixed by the Fourier relation, not fitted.  ISPG therefore
...
\begin{enumerate}
  \item $a_0=cH/(2\pi)$ is derived, not a free parameter.
...
sector is GR-identical by construction (Sec.~\ref{sec:consistency}).
...
\begin{itemize}
  ...
  \item \textbf{Cosmology.}  Superfluid dark matter modifies the
    ...
    ISPG leaves the linear cosmological sector GR-identical.
\end{itemize}
...
\begin{itemize}
  \item \textbf{$a_0$ precision.}  Singh's $a_0=c^2/(\xi\,\ell_{\rm dS})$
    contains an undetermined $O(1)$ factor $\xi$; ISPG fixes the
    coefficient to $1/(2\pi)$ from the Hubble coherence boundary
    and resolves the remaining 15\% offset via the vortex-lag
    mechanism (Sec.~\ref{sec:vortex_lag}).
...
dark-matter particle, preserves GR-identical linear cosmology,
...
and the 15\% $a_0$ offset resolved by the vortex-lag effect
...
selected by consistency, or observationally constrained.
...
$a_0\propto cH$; the vortex-lag resolution of the 15\%
coefficient offset via time-averaged $H_{\rm eff}$
(Sec.~\ref{sec:vortex_lag}), yielding $a_0^{\rm eff}=1.20\times
10^{-10}\;\mathrm{m\,s^{-2}}$ for $z_{\rm form}\approx 0.58$;
the Fourier $m=0$ dominance; the Bessel
...
observed cosmic flows (Laniakea, cosmic web, bulk flows) in the
space substance $\to$ vortex formation (Kelvin--Helmholtz
instability) $\to$ central rarefaction $\to$ MOND.  Galaxy
morphology (spiral, elliptical, irregular, dwarf) maps onto
fluid-dynamical vortex topology (laminar planar, convective
poloidal, chaotic, small-scale eddy).  The MOND scale $a_0$ is
common to all types because it is set by the cosmological
...
\]
together with: the vortex-lag resolution
$a_0^{\rm eff}=cH_{\rm eff}/(2\pi)$ with
$H_{\rm eff}=\ln(1+z_{\rm form})/t_{\rm lookback}$, predicting
galaxy-age-dependent $a_0$ and correlated BTFR scatter
(Sec.~\ref{sec:vortex_lag});
...
\bibitem{MIGHTEE2025}
P.~Villanueva-Domingo \textit{et al.},
``The first satisfactory measurement of the baryonic Tully--Fisher relation at $z\sim1$,''
preprint (2025).
